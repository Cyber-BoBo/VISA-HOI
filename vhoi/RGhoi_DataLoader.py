...
class SubclipSamplingConfig:
    window_size: int = 32
    sampling_mode: str = "random"
    stride: int = 1
    seed: int = 42
    drop_short_videos: bool = False


@dataclass(frozen=True)
class OnlineSlidingWindowConfig:
    window_size: int = 32


def _infer_valid_lengths_from_y(y: torch.Tensor) -> torch.Tensor:
    if y.ndim < 2:
        raise ValueError(f"y must have at least 2 dims (N,T,...) but got {tuple(y.shape)}")
    n, t = y.shape[0], y.shape[1]
    if y.ndim == 2:
        valid = y != -1
    else:
        valid = (y != -1).reshape(int(n), int(t), -1).any(dim=2)
    lengths = torch.zeros((n,), dtype=torch.long)
    for i in range(n):
        idx = torch.nonzero(valid[i], as_tuple=False).view(-1)
        lengths[i] = int(idx[-1].item() + 1) if idx.numel() else 0
    return lengths.clamp(min=0, max=t)


def _select_temporal_reference_length(tensors: Sequence[torch.Tensor]) -> int:
    for t in tensors:
        if t.ndim >= 2:
            return int(t.shape[1])
    raise ValueError("No temporal tensor found (expected at least one tensor with ndim>=2).")


def _crop_time(t: torch.Tensor, start: int, window_size: int, temporal_length: int) -> torch.Tensor:
    if t.ndim < 2 or int(t.shape[1]) != temporal_length:
        return t
    end = start + window_size
    sliced = t[:, start:end, ...]
    missing = window_size - int(sliced.shape[1])
    if missing <= 0:
        return sliced
...
class RGHOISubclipDataset(Dataset):
    def __init__(
        self,
        xs: List[torch.Tensor],
        ys: List[torch.Tensor],
        sampling: SubclipSamplingConfig,
    ):
        if not xs:
            raise ValueError("xs must be non-empty.")
        if not ys:
            raise ValueError("ys must be non-empty.")
        if sampling.window_size <= 0:
        ...
        self.temporal_length = _select_temporal_reference_length(list(xs) + list(ys))

        y_ref = ys[0]
        self.lengths = _infer_valid_lengths_from_y(y_ref)

        self._video_indices = torch.arange(int(y_ref.shape[0]), dtype=torch.long)
        if sampling.drop_short_videos:
            keep = self.lengths >= sampling.window_size
            self._video_indices = self._video_indices[keep]
            self.lengths = self.lengths[keep]

        self._sliding_index: Optional[List[Tuple[int, int]]] = None
        self._det_starts: Optional[torch.Tensor] = None
        if sampling.sampling_mode == "sliding":
            self._sliding_index = self._build_sliding_index()
        elif sampling.sampling_mode == "deterministic":
            self._det_starts = self._build_deterministic_starts()
        elif sampling.sampling_mode == "random":
            pass
        else:
            raise ValueError(
                f"sampling_mode must be one of: random, deterministic, sliding; got {sampling.sampling_mode}"
            )

    def _build_sliding_index(self) -> List[Tuple[int, int]]:
        index: List[Tuple[int, int]] = []
        w, stride = self.sampling.window_size, self.sampling.stride
        for local_i, video_i in enumerate(self._video_indices.tolist()):
            length = int(self.lengths[local_i].item())
            if length <= 0:
                continue
            if length < w:
                index.append((video_i, 0))
                continue
            last = length - w
            ...
            if starts and starts[-1] != last:
                starts.append(last)
            for s in starts:
                index.append((video_i, s))
        return index

    def _build_deterministic_starts(self) -> torch.Tensor:
        w = self.sampling.window_size
        rng = np.random.RandomState(self.sampling.seed)
        starts = []
        for length in self.lengths.tolist():
            ...
            if length <= w:
                ...
                starts.append(int(rng.randint(0, length - w + 1)))
        return torch.tensor(starts, dtype=torch.long)

    def __len__(self) -> int:
        if self._sliding_index is not None:
            return len(self._sliding_index)
        return int(self._video_indices.numel())

    def _sample_start(self, local_index: int) -> Tuple[int, int]:
        if self._sliding_index is not None:
            return self._sliding_index[local_index]

        video_i = int(self._video_indices[local_index].item())
        length = int(self.lengths[local_index].item())
        w = self.sampling.window_size

        if length <= w:
            return video_i, 0

        if self.sampling.sampling_mode == "deterministic":
            assert self._det_starts is not None
            return video_i, int(self._det_starts[local_index].item())

        start = int(np.random.randint(0, length - w + 1))
        return video_i, start

    def __getitem__(self, index: int):
        video_i, start = self._sample_start(index)
        w = self.sampling.window_size
        t_total = self.temporal_length

        xs = [_crop_time(x[video_i : video_i + 1], start, w, t_total).squeeze(0) for x in self.xs]
        ys = [_crop_time(y[video_i : video_i + 1], start, w, t_total).squeeze(0) for y in self.ys]
        return tuple(xs + ys)


def _find_steps_tensor_index(xs: Sequence[torch.Tensor]) -> int:
    candidate_indices = [i for i, t in enumerate(xs) if t.ndim == 1]
    if not candidate_indices:
        raise ValueError("Could not find steps_per_example tensor in xs (expected a 1D tensor).")
    return candidate_indices[-1]


def _select_temporal_reference_tensor(xs: Sequence[torch.Tensor]) -> torch.Tensor:
    for t in xs:
        if t.ndim >= 2:
            return t
    raise ValueError("No temporal tensor found in xs (expected at least one tensor with ndim>=2).")


def _window_indices_for_time(t: int, window_size: int) -> torch.Tensor:
    if t < 0:
        raise ValueError("t must be >= 0")
    if window_size <= 0:
        raise ValueError("window_size must be > 0")
    if t < window_size - 1:
        pad = [0] * (window_size - (t + 1))
        tail = list(range(0, t + 1))
        return torch.tensor(pad + tail, dtype=torch.long)
    start = t - window_size + 1
    return torch.arange(start, t + 1, dtype=torch.long)


class RGHOIOnlineWindowDataset(Dataset):
    def __init__(self, xs: List[torch.Tensor], config: OnlineSlidingWindowConfig):
        if not xs:
            raise ValueError("xs must be non-empty.")
        ...
        self.xs = xs
        self.config = config
        self.steps_index = _find_steps_tensor_index(xs)
        self.temporal_ref = _select_temporal_reference_tensor(xs)
        self.temporal_length = int(self.temporal_ref.shape[1])
        self.num_videos = int(self.temporal_ref.shape[0])

        steps = xs[self.steps_index]
        if int(steps.shape[0]) != self.num_videos:
            raise ValueError("steps_per_example does not match num_videos.")
        self.steps_down = steps.to(torch.long).clamp(min=0, max=self.temporal_length)
        self.max_steps_down = int(self.steps_down.max().item()) if self.num_videos else 0

        self._index: List[Tuple[int, int]] = []
        for v in range(self.num_videos):
            length = int(self.steps_down[v].item())
            for t in range(length):
                self._index.append((v, t))

    def __len__(self) -> int:
        return len(self._index)

    def __getitem__(self, index: int):
        video_i, t = self._index[index]
        w = self.config.window_size
        idx = _window_indices_for_time(t, w)

        out: List[torch.Tensor] = []
        for i, x in enumerate(self.xs):
            if i == self.steps_index:
                out.append(torch.tensor(float(w), dtype=x.dtype))
                continue

            x_v = x[video_i]
            if x.ndim >= 2 and int(x.shape[1]) == self.temporal_length:
                out.append(x_v.index_select(dim=0, index=idx))
            else:
                out.append(x_v)

        out.append(torch.tensor(video_i, dtype=torch.long))
        out.append(torch.tensor(t, dtype=torch.long))
        return tuple(out)


def assemble_windowed_tensors(
    data: list,
    model_name: str,
    ...
    if dataset_name == "cad120":
        ...
        xs, ys = assemble_bimanual_tensors(
            data,
            model_name=model_name,
            sigma=sigma,
            downsampling=downsampling,
        ...
        xs, ys = assemble_mphoi_tensors(
            data,
            model_name=model_name,
            input_caption_action_match=input_caption_action_match,
            sigma=sigma,
            downsampling=downsampling,
            test_data=test_data,
        )
...
def create_data_loader(data, model_name: str, model_input_type: str, dataset_name: str, batch_size: int, shuffle: bool,
                       ...
                       downsampling: int = 1, test_data: bool = False, input_caption_action_match: bool = False):
...
def create_subclip_data_loader(
    data: list,
    model_name: str,
    model_input_type: str,
    dataset_name: str,
    batch_size: int,
    shuffle: bool,
    sampling: Optional[SubclipSamplingConfig] = None,
    ...
    xs_np, ys_np = assemble_windowed_tensors(
        data=data,
        model_name=model_name,
        model_input_type=model_input_type,
    ...
    xs_np, scalers = maybe_scale_input_tensors(xs_np, model_name, scaling_strategy=scaling_strategy, scalers=scalers)
    xs_np = [np.nan_to_num(ix, copy=False, nan=0.0) for ix in xs_np]
    xs_t = numpy_to_torch(*xs_np)
    ys_t = numpy_to_torch(*ys_np)

    dataset = RGHOISubclipDataset(xs=list(xs_t), ys=list(ys_t), sampling=sampling)
    loader = DataLoader(
        dataset,
        batch_size=batch_size,
        shuffle=shuffle if sampling.sampling_mode != "sliding" else False,
        num_workers=num_workers,
        pin_memory=False,
        drop_last=False,
    )
    segmentations = assemble_segmentations(data, model_name, dataset_name=dataset_name)
    return loader, scalers, segmentations


def create_online_window_data_loader(
    data: list,
    model_name: str,
    model_input_type: str,
    dataset_name: str,
    ...
    xs_np, scalers = maybe_scale_input_tensors(xs_np, model_name, scaling_strategy=scaling_strategy, scalers=scalers)
    xs_np = [np.nan_to_num(ix, copy=False, nan=0.0) for ix in xs_np]
    xs_t = list(numpy_to_torch(*xs_np))
    ys_t = list(numpy_to_torch(*ys_np))

    dataset = RGHOIOnlineWindowDataset(xs=xs_t, config=OnlineSlidingWindowConfig(window_size=window_size))
    loader = DataLoader(
        dataset,
        batch_size=batch_size,
        shuffle=False,
    ...
    segmentations = assemble_segmentations(data, model_name, dataset_name=dataset_name)
    return loader, scalers, segmentations, ys_t



#====================================================
#----------------------------------------------------
"""CAD120数据集的Training data加载"""
def load_cad120_training_data(data_path, model_name, model_input_type, test_subject_id, video_id_to_subject_id,
    ...
    for video_id, video_data in data.items():
        ...
        if root is not None:
            training_data_datum = [root[video_id + '/skeleton'][:], root[video_id + '/objects'][:],
                                   ...
                                   video_data]
            # print(root[video_id + '/skeleton'][0].shape)
            # np.savetxt('test.txt',np.array(root[video_id + '/skeleton'][:]),fmt="%f")
            # plt.imshow(root[video_id + '/skeleton'][:])
            # time.sleep(100)
        else:
    ...
    if debug:
        ...
        val_data = val_data[:1]
    train_loader, scalers, _ = create_subclip_data_loader(training_data, model_name, model_input_type, 'cad120',
                                                  batch_size=batch_size, shuffle=True,
                                                  scaling_strategy=scaling_strategy, sigma=sigma,
                                                  downsampling=downsampling, test_data=False)
    val_loader, _, _ = create_subclip_data_loader(val_data, model_name, model_input_type, 'cad120', batch_size=len(val_data),
...
def load_bimanual_training_data(data_path, data_path_zarr, data_path_bbs_zarr, data_path_hps_zarr, model_name: str, model_input_type: str,
    ...
    if debug:
        training_data = training_data[:4]
        val_data = val_data[:1]
    train_loader, scalers, _ = create_subclip_data_loader(training_data, model_name, model_input_type, 'bimanual',
                                                  batch_size=batch_size, shuffle=True,
                                                  scaling_strategy=scaling_strategy, sigma=sigma,
                                                  downsampling=downsampling, test_data=False)
    val_loader, _, _ = create_subclip_data_loader(val_data, model_name, model_input_type, 'bimanual', batch_size=len(val_data),
                                          shuffle=False, scalers=scalers, sigma=sigma, downsampling=downsampling,
                                          test_data=False)
    input_size = input_size_from_data_loader(train_loader, model_name, model_input_type)
    data_info = {'input_size': input_size}
...
def load_mphoi_training_data(data_path, data_path_zarr, data_path_obbs_zarr, data_path_hbbs_zarr, data_path_hps_zarr, model_name: str, 
    ...
    with open(data_path, mode='rb') as f:   # 以2进制方式读取
        data = json.load(f)
    root = zarr.open(data_path_zarr, mode='r')                      # faster_rcnn_featrues
    root_obbs = zarr.open(data_path_obbs_zarr, mode='r')            # object bounding boxes
    root_hbbs = zarr.open(data_path_hbbs_zarr, mode='r')            # human bounding boxes
    root_hps = zarr.open(data_path_hps_zarr, mode='r')              # human pose
    training_data = []
    ...
    if debug:       # Debug用
        training_data = training_data[:4]
        val_data = val_data[:1]
    train_loader, scalers, _ = create_subclip_data_loader(training_data, model_name, model_input_type, 'mphoi',
                                                  batch_size=batch_size, shuffle=True,
                                                  scaling_strategy=scaling_strategy, sigma=sigma,
                                                  downsampling=downsampling, test_data=False)
    val_loader, _, _ = create_subclip_data_loader(val_data, model_name, model_input_type, 'mphoi', batch_size=len(val_data),
                                          shuffle=False, scalers=scalers, sigma=sigma, downsampling=downsampling,
                                          test_data=False)
    input_size = input_size_from_data_loader(train_loader, model_name, model_input_type)
    data_info = {'input_size': input_size}
...
def load_cad120_testing_data(data_path, model_name, model_input_type, test_subject_id, video_id_to_subject_id,
    ...
    test_loader, _, segmentations = create_subclip_data_loader(testing_data, model_name, model_input_type, 'cad120',
...
def load_bimanual_testing_data(data_path, data_path_zarr, data_path_bbs_zarr, data_path_hps_zarr, model_name: str, model_input_type: str,
    ...
    for video_id, hands_ground_truth in data.items():
        ...
        testing_data.append([left_hand_features, right_hand_features, object_features, hands_ground_truth,
                             left_hand_bbs, right_hand_bbs, objects_bbs, left_hand_hps, right_hand_hps])
        test_ids.append(video_id)
    test_loader, _, segmentations = create_subclip_data_loader(testing_data, model_name, model_input_type, 'bimanual',
                                                       batch_size=batch_size, shuffle=False, scalers=scalers,
                                                       downsampling=downsampling, test_data=True)
    input_size = input_size_from_data_loader(test_loader, model_name, model_input_type)
    data_info = {'input_size': input_size}
...
def load_mphoi_testing_data(data_path, data_path_zarr, data_path_obbs_zarr, data_path_hbbs_zarr, data_path_hps_zarr,
    ...
    for video_id, human_ground_truth in data.items():
        ...
        testing_data.append([Human1_features, Human2_features, object_features, human_ground_truth,
                            Human1_bbs, Human2_bbs, objects_bbs, Human1_hps, Human2_hps])
        test_ids.append(video_id)
    test_loader, _, segmentations = create_subclip_data_loader(testing_data, model_name, model_input_type, 'mphoi',
                                                       batch_size=batch_size, shuffle=False, scalers=scalers,
                                                       downsampling=downsampling, test_data=True)
    input_size = input_size_from_data_loader(test_loader, model_name, model_input_type)
    data_info = {'input_size': input_size}
...
#----------------------------------------------------
def load_testing_data_online_windows(
    data,
    model_name: str,
    model_input_type: str,
    batch_size: int,
    ...
    **kwargs,
    ...
    dataset_name = data.get('name', default_value='cad120').lower()
    ...
    if 'BimanualActions' in data_path:
        ...
        testing_data, test_ids = [], []
        for video_id, hands_ground_truth in annotations.items():
            subject, task, take = video_id.split(sep='-')
            subject_id = int(subject.split(sep='_')[1])
            if subject_id != test_subject_id:
                continue
        ...
        window_loader, _, segmentations, full_targets = create_online_window_data_loader(
            testing_data,
            model_name,
            model_input_type,
            dataset_name="bimanual",
            batch_size=batch_size,
        ...
        testing_data, test_ids = [], []
        for video_id, human_ground_truth in annotations.items():
            subject_id, task, take = video_id.split(sep='-')
            if subject_id != test_subject_id:
                continue
            Human1_features = root[video_id]['Human1'][:]
            ...
            if data_path_hbbs_zarr.split('.')[-2].endswith('rtdetr'):
                Human1_bbs = root_hbbs[video_id]['Human1'][:].squeeze(1)
                Human2_bbs = root_hbbs[video_id]['Human2'][:].squeeze(1)
            else:
                Human1_bbs = root_hbbs[video_id]['Human1'][:]
                Human2_bbs = root_hbbs[video_id]['Human2'][:]
            objects_bbs = root_obbs[video_id]['objects'][:]
        ...
        window_loader, _, segmentations, full_targets = create_online_window_data_loader(
            ...
            scalers=scalers,
            downsampling=downsampling,
            input_caption_action_match=kwargs.get('input_caption_action_match', False),
        )
        ...
        testing_data, test_ids = [], []
        for video_id, video_data in cad_data.items():
            subject_id = video_id_to_subject_id.get(video_id, None)
            if subject_id is None or subject_id != test_subject_id:
                continue
            if root is not None:
                testing_data_datum = [
                    root[video_id + '/skeleton'][:],
                    root[video_id + '/objects'][:],
                    root[video_id + '/skeleton_bounding_box'][:],
                    root[video_id + '/objects_bounding_box'][:],
                    root[video_id + '/skeleton_2d_pose'][:],
                    video_data,
    ...
    data_info = {'input_size': input_size_from_data_loader(window_loader, model_name, model_input_type)}
    return window_loader, data_info, segmentations, test_ids, full_targets, downsampling, dataset_name


#----------------------------------------------------
""" 同load_training_data,加载测试集数据 """
...
def assemble_segmentations(data, model_name: str, dataset_name: str):
    segmentations = None
    if model_name in {'PATHoi', 'GT3-HOI', 'TIGAhoi'}:
        if dataset_name.lower() == 'cad120':
...
def maybe_scale_input_tensors(x: list, model_name: str, scaling_strategy: Optional[str] = None,
    ...
    if there_are_no_scalers:
        ...
        if scaling_strategy is None:
            return x, scalers
    if model_name in {'PATHoi', 'GT3-HOI','TIGAhoi', 'bimanual_baseline', 'cad120_baseline'}:
...
def assemble_tensors(data: list, model_name: str, model_input_type: str, sigma: float = 0.0, downsampling: int = 1,
                     test_data: bool = False):
    if model_name in {'PATHoi',  'GT3-HOI', 'TIGAhoi', 'cad120_baseline'}:
...
def assemble_mphoi_tensors(data: list, model_name: str, sigma: float = 0.0, downsampling: int = 1,
                              test_data: bool = False, input_caption_action_match: bool = False):
    ...
    if model_name in {'PATHoi', 'GT3-HOI', 'TIGAhoi', 'GitHOI'}:
        if sigma:
            ys[2] = ignore_last_step_end_flag_general(ys[2])
        ys[2] = smooth_segmentation(ys[2], sigma)
        ys_budget = ys[2]
        ...
        xs_steps = assemble_num_steps(data, downsampling=downsampling)
        xs = xs[:1] + xs_objects + xs[1:] + [xs_hh_dists, xs_ho_dists, xs_oo_dists, xs_steps]
        if input_caption_action_match:
            xs_caption, xs_action = assemble_mphoi_caption_action_features(data, downsampling=downsampling)
            xs = xs + [xs_caption, xs_action]
        ys = [ys_budget] + ys[2:] + ys[:2]
        ys += ys[-2:]
    else:
...
def assemble_mphoi_frame_level_recurrent_human(data, downsampling: int = 1, test_data: bool = False):
    ...
    x_obb= []
    max_len, max_len_downsampled = 0, 0
    for Human1, Human2, _, _, _, _, objects_bounding_box, Human1_pose, Human2_pose, *_ in data:
        # human feature
    ...
    for i, video in enumerate(xs_h1):
        ...
        for j, frame in enumerate(video):
            ...
            if j+1 < len(video):
                ...
                # h1p_velo = np.zeros((10, 2))
                # h2p_velo = np.zeros((10, 2))
                # obb_velo = np.zeros((8, 2))
    ...
    y_rec_hs = np.full([len(x_hs), max_len, 2], fill_value=-1, dtype=np.int64)
    y_pred_hs = np.full_like(y_rec_hs, fill_value=-1)
    for m, (_, _, _, video_hands_ground_truth, _, _, _, _, _, *_) in enumerate(data):
        # Human1
...
def assemble_mphoi_frame_level_recurrent_objects(data, downsampling: int = 1):
    xs_objects = []
    max_len, max_len_downsampled, max_num_objects = 0, 0, 0
    for _, _, objects, _, _, _, _, _, _, *rest in data:
        max_len = max(max_len, objects.shape[0])
        max_num_objects = max(max_num_objects, objects.shape[1])
        objects = objects[downsampling - 1::downsampling]
        max_len_downsampled = max(max_len_downsampled, objects.shape[0])
...
def assemble_mphoi_human_human_distances(data, downsampling: int = 1):
    ...
    max_len, max_num_humans = 0, 2
    all_dists = []
    for _, _, _, _, h1_bb, h2_bb, _, _, _, *rest in data:
...
def assemble_mphoi_human_object_distances(data, downsampling: int = 1):
    ...
    max_len, max_num_humans, max_num_objects = 0, 2, 0
    h1_dists, h2_dists = [], []
    for _, _, _, _, h1_bb, h2_bb, obj_bbs, _, _, *rest in data:
        h1_bb = h1_bb[downsampling - 1::downsampling]
        h1_centroids = compute_centroid(h1_bb) / mphoi_dims
        h2_bb = h2_bb[downsampling - 1::downsampling]
        h2_centroids = compute_centroid(h2_bb) / mphoi_dims
...
def assemble_mphoi_object_object_distances(data, downsampling: int = 1):
    ...
    max_len, max_num_objects = 0, 0
    all_dists = []
    for _, _, _, _, _, _, obj_bbs, _, _, *rest in data:
        obj_bbs = obj_bbs[downsampling - 1::downsampling]
...
def assemble_mphoi_caption_action_features(data, downsampling: int = 1):
    captions = []
    lengths = []
    action_feature = None
    for Human1, *_middle, caption_feature, action_feat in data:
        x_h1_down = Human1[downsampling - 1::downsampling]
        expected_len = int(x_h1_down.shape[0])
        cap = np.asarray(caption_feature, dtype=np.float32)
        if cap.ndim != 2 or cap.shape[-1] != 768:
            raise ValueError(f"caption_feature must have shape (T,768) but got {cap.shape}")
        if cap.shape[0] == expected_len:
            cap_use = cap
        elif cap.shape[0] == Human1.shape[0]:
            cap_use = cap[downsampling - 1::downsampling]
        else:
            cap_use = cap[:expected_len]
            if cap_use.shape[0] < expected_len:
                pad = np.zeros((expected_len - cap_use.shape[0], 768), dtype=np.float32)
                cap_use = np.concatenate([cap_use, pad], axis=0)
        captions.append(cap_use)
        lengths.append(int(cap_use.shape[0]))
        if action_feature is None:
            action_feature = np.asarray(action_feat, dtype=np.float32)
    max_len = max(lengths) if lengths else 0
    x_caption = np.full((len(captions), max_len, 768), fill_value=np.nan, dtype=np.float32)
    for i, cap in enumerate(captions):
        x_caption[i, : cap.shape[0], :] = cap
    if action_feature is None:
        raise ValueError("Missing action_feature in data items.")
    if action_feature.ndim != 2 or action_feature.shape[-1] != 768:
        raise ValueError(f"action_feature must have shape (K,768) but got {action_feature.shape}")
    x_action = np.broadcast_to(action_feature[None, ...], (len(captions),) + action_feature.shape).copy()
    return x_caption, x_action
...
def determine_num_classes(model_name: str, model_input_type: str, dataset_name: str):
    if model_name in {'GitHOI', 'TIGAhoi', 'bimanual_baseline', 'cad120_baseline'}:
...
def load_images_dict(dataset_name = 'mphoi'):
    image_loader = {
        'mphoi': load_mphoi_images_dict,
        'bimanual': load_bimanual_images_dict
    }
    return image_loader[dataset_name]()

def load_mphoi_images_dict(image_dir = '/mnt/1c714c0e-2ec6-4b54-b86f-8ab77eaea676/study/Dataset/MPHOI-72/mphoi_raw_datas/MPHOI_rgb'):
    image_list_dict = {}
    for subject in MPHOI_72_SUBJECT:
        for task in MPHOI_72_TASK:
            for take in MPHOI_72_TAKE:
                current_video_dir = os.path.join(image_dir, subject, task, take)
                current_video_name = subject + '-' + task + '-' + take
                image_list_dict[current_video_name] = os.path.join(image_dir, current_video_dir)

    for key, rgb_data in image_list_dict.items():
        image_list_dict[key] = []
        chunk_folder = [os.path.join(rgb_data, f) for f in os.listdir(rgb_data)]
        chunk_folder.sort(key=lambda x: int(x.split('.')[0].split('_')[-1]))
        image_list_dict[key] = chunk_folder

    return image_list_dict

def load_bimanual_images_dict(image_dir = '/mnt/1c714c0e-2ec6-4b54-b86f-8ab77eaea676/study/Dataset/BimanualActions/bimacs_raw_datas'):
    image_chunk_list_dict = {}
    image_list_dict = {}
    for subject in BIMACS_SUBJECT:
        for task in BIMACS_TASK:
            for take in BIMACS_TAKE:
                current_video_dir = os.path.join(image_dir, subject, task, take, 'rgb')
                current_video_name = subject + '-' + task + '-' + take
                image_chunk_list_dict[current_video_name] = os.path.join(image_dir, current_video_dir)

    
    for key, rgb_data in image_chunk_list_dict.items():
        image_list_dict[key] = []
        chunk_folder = [os.path.join(rgb_data, f) for f in os.listdir(rgb_data) 
                        if f.startswith('chunk_') and os.path.isdir(os.path.join(rgb_data, f))]
        chunk_folder.sort(key=lambda x: int(x.split('_')[-1]))

        for chunk in chunk_folder:
            frames = [os.path.join(chunk, f) for f in os.listdir(chunk) if f.startswith('frame_')]
            frames.sort(key=lambda x: int(x.split('.')[0].split('_')[-1]))
            image_list_dict[key] += frames
    return image_list_dict
...
def _smoke_test():
    n, t, f = 3, 50, 8
    xs = [torch.randn(n, t, f)]
    ys = [torch.full((n, t), fill_value=-1, dtype=torch.long)]
    ys[0][0, :45] = 1
    ys[0][1, :20] = 2
    ys[0][2, :50] = 3

    dataset = RGHOISubclipDataset(
        xs=xs,
        ys=ys,
        sampling=SubclipSamplingConfig(window_size=32, sampling_mode="sliding", stride=16, seed=0),
    )
    loader = DataLoader(dataset, batch_size=2, shuffle=False)
    batch = next(iter(loader))
    if batch[0].shape[1] != 32 or batch[1].shape[1] != 32:
        raise RuntimeError("Windowed batch does not have expected temporal length.")

    y3 = torch.full((n, t, 2), fill_value=-1, dtype=torch.long)
    y3[0, :10, 0] = 1
    y3[0, :10, 1] = -1
    y3[1, :25, 0] = -1
    y3[1, :25, 1] = 2
    y3[2, :50, :] = 3
    lengths = _infer_valid_lengths_from_y(y3)
    if lengths.tolist() != [10, 25, 50]:
        raise RuntimeError(f"Unexpected inferred lengths for 3D y: {lengths.tolist()}")
