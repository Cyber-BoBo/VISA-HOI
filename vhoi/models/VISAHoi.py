...
from pyrutils.torch.modules.modules_gcn import GeometricGraph
from pyrutils.torch.modules.modules_iim import IIM
from pyrutils.torch.modules.modules_cam import ActionMatch


class VISAhoi(nn.Module):
    def __init__(self, input_size: tuple, num_classes: tuple, hidden_size: int = 128,
                 ...
                 human_node: int = 9, human_num: int = 2, object_num: int = 4,
                 iim_num_heads: int = 8, iim_layer_num: int = 3, cam_layer_num: int = 2,
                ...
                #  message_semantic_to_objects: bool = True, message_semantic_to_human: bool = False,
                 caption_message: bool = False,
                 ...
                 input_caption_action_match: bool = False,
                 positional_encoding_style: str = 'embedding', cat_level_states: bool = False,
                 share_level_mlps: bool = False, bias: bool = True, delta: bool = True,
                 ablation_GE: bool = False, ablation_MA: bool = False
                 ):
        ...
        Arguments:
            ...
            是否对离散更新应用本地最大筛选器。这样可以避免出现连续离散更新的情况
        ...
        """
        super(VISAhoi, self).__init__()
        human_input_size, object_input_size = input_size        # 人和物体的输入大小
        num_subactivities, num_affordances = num_classes        # 子活动类型数量与
        ...
        self.graph_node_ho = human_node+object_num*2
        self.graph_node_hh = human_node*human_num
        self.graph_node_oo = object_num*2
        self.graph_node_global = human_num + object_num

        self.delta = delta
        self.iim_num_heads = iim_num_heads
        self.iim_layer_num = iim_layer_num
        self.cam_layer_num = cam_layer_num
        ...
        self.caption_message = caption_message
        self.message_type = message_type                    # 消息传递类型
        self.message_granularity = message_granularity      # 消息传递粒度
        self.message_aggregation = message_aggregation      # 消息聚合
        self.attention_style = attention_style              # 注意力形式
        ...
        self.input_caption_action_match = input_caption_action_match

        self.ablation_GE = ablation_GE
        self.ablation_MA = ablation_MA
        ...
        if self.caption_message:
            self.caption_proj = nn.Linear(768, hidden_size, bias=bias)
            self.action_proj = nn.Linear(768, hidden_size, bias=bias)
            self.visual_proj = nn.Linear(2*hidden_size, hidden_size, bias=bias)
            self.caption_action_match = ActionMatch(layers=self.cam_layer_num, embed_dim=hidden_size)
            self.visual_action_match = ActionMatch(layers=self.cam_layer_num, embed_dim=hidden_size)

            self.caption_dim_redu_recognition = nn.Sequential(
                nn.Linear(hidden_size, hidden_size//16, bias=bias),
                nn.LeakyReLU(negative_slope=0.1),
                nn.LayerNorm(hidden_size//16),
                nn.Linear(hidden_size//16, 1, bias=bias),
            )
            self.caption_dim_redu_prediction = nn.Sequential(
                nn.Linear(hidden_size, hidden_size//16, bias=bias),
                nn.LeakyReLU(negative_slope=0.1),
                nn.LayerNorm(hidden_size//16),
                nn.Linear(hidden_size//16, 1, bias=bias),
            )
            # self.caption_dim_redu_recognition = build_mlp([hidden_size, hidden_size//16, 1], ['leakyrelu', 'leakyrelu'], bias=bias, norm=True)
            # self.caption_dim_redu_prediction = build_mlp([hidden_size, hidden_size//16, 1], ['leakyrelu', 'leakyrelu'], bias=bias, norm=True)
            # self.caption_action_proj_mlp = build_mlp([2*hidden_size, hidden_size], ['leakyrelu'], bias=bias)
            self.caption_action_fusion_weight_prediction = nn.Parameter(torch.tensor(-0.2), requires_grad=True)
            self.caption_action_fusion_weight_recognition = nn.Parameter(torch.tensor(-0.2), requires_grad=True)


            # self.caption_dim_redu_recognition = build_mlp([hidden_size, hidden_size//16, 1], ['leakyrelu', 'sigmoid'], bias=bias)
            # self.caption_dim_redu_prediction = build_mlp([hidden_size, hidden_size//16, 1], ['leakyrelu', 'sigmoid'], bias=bias)
            # # self.caption_action_proj_mlp = build_mlp([2*hidden_size, hidden_size], ['leakyrelu'], bias=bias)
            # self.caption_action_fusion_weight = nn.Parameter(torch.tensor(0.05), requires_grad=True)
        
        # Geometric Graph
        # geometry
        if self.ablation_GE:
            self.geometry_embedding_gcn = GeometricGraph(human_num*human_node + object_num*2, 4, 128, self.delta)
            self.geometry_embedding_mlp = build_mlp([(human_num*human_node + object_num*2)*128, hidden_size*4, hidden_size], ['relu', 'relu'], bias=bias)
        
        else:
            self.geometry_embedding_gcn_ho1 = GeometricGraph(self.graph_node_ho, 4, 128, self.delta)
            self.geometry_embedding_mlp_ho1 = build_mlp([self.graph_node_ho*128, hidden_size*4, hidden_size], ['relu','relu'], bias=bias)
            self.geometry_embedding_gcn_oo = GeometricGraph(self.graph_node_oo, 4, 128, self.delta)
            self.geometry_embedding_mlp_oo = build_mlp([self.graph_node_oo*128, hidden_size*4, hidden_size], ['relu','relu'], bias=bias)
            self.geometry_embedding_gcn_global = GeometricGraph(self.graph_node_global, 4, 128, self.delta)
            self.geometry_embedding_mlp_global = build_mlp([self.graph_node_global*128, hidden_size*4, hidden_size], ['relu','relu'], bias=bias)
            self.geometry_embedding_gcn_hh = GeometricGraph(self.graph_node_hh, 4, 128, self.delta)
            self.geometry_embedding_mlp_hh = build_mlp([self.graph_node_hh*128, hidden_size*4, hidden_size], ['relu','relu'], bias=bias)

            if self.human_num == 2:
                self.geometry_embedding_gcn_ho2 = GeometricGraph(self.graph_node_ho, 4, 128, self.delta)
                self.geometry_embedding_mlp_ho2 = build_mlp([self.graph_node_ho*128, hidden_size*4, hidden_size], ['relu','relu'], bias=bias)
                
            self.geometry_ho_weight = nn.Parameter(torch.tensor(0.5),requires_grad=True)
            self.geometry_oo_weight = nn.Parameter(torch.tensor(0.5),requires_grad=True)


        self.geometry_proj_mlp = build_mlp([2*hidden_size, hidden_size], ['relu'], bias=bias)
        ...
        # if self.message_semantic_to_human or self.message_semantic_to_objects:
        #     self.semantic_proj_mlp = build_mlp([2*hidden_size, hidden_size], ['relu'], bias=bias)
        #     self.semantic_bd_rnn = nn.GRU(hidden_size, hidden_size, num_layers=1, bias=bias, batch_first=True,
        #                                 bidirectional=True)
        #     self.semantic_bd_embedding_mlp = build_mlp([2 * hidden_size, hidden_size], ['relu'], bias=bias)
        ...
        self.human_bd_rnn = nn.GRU(hidden_size, hidden_size, num_layers=1, bias=bias, batch_first=True,
                                   bidirectional=True)      # 门控循环单元(emb_dim, hid_dim)
        ...
        if message_humans_to_human:
            if message_type in {'v1', 'relational'}:
                ...
                if message_type in {'v2', 'non-relational'}:
                    ...
                    self.humans_to_human_iim = IIM(2*hidden_size, hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                    if message_segment:
                        self.humans_to_human_segment_iim = IIM(hidden_size, hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
        ...
        if message_human_to_objects:
            if message_type in {'v1', 'relational'}:
                ...
                if message_type in {'v2', 'non-relational'}:
                    ...
                    self.humans_to_object_iim = IIM(2*hidden_size, hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                    if message_segment:
                        self.humans_to_object_segment_iim = IIM(hidden_size, hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                if message_aggregation in {'att', 'attention'}:
                    if attention_style in {'v4', 'general'}:
        ...
        if message_objects_to_human:
            if message_type in {'v1', 'relational'}:
                ...
                if message_type in {'v2', 'non-relational'}:
                    ...
                    self.objects_to_human_iim = IIM(2*hidden_size, hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                    if message_segment:
                        self.objects_to_human_segment_iim = IIM(hidden_size, hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                if message_aggregation in {'att', 'attention'}:
                    if attention_style in {'v4', 'general'}:
        ...
        if message_objects_to_object:
            if message_type in {'v1', 'relational'}:
                ...
                if message_type in {'v2', 'non-relational'}:
                    ...
                    self.objects_to_object_iim = IIM(2*hidden_size,hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                    if message_segment:
                        self.objects_to_object_segment_iim = IIM(hidden_size, hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                if message_aggregation in {'att', 'attention'}:
                    if attention_style in {'v4', 'general'}:
        ...
        if message_geometry_to_human:
            if message_type in {'v1', 'relational'}:
                ...
                if message_type in {'v2', 'non-relational'}:
                    ...
                    self.geometry_to_human_iim = IIM(2*hidden_size, hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                    if message_segment:
                        self.geometry_to_human_segment_iim = IIM(hidden_size, hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                if message_aggregation in {'att', 'attention'}:
                    if attention_style in {'v4', 'general'}:
        ...
        if message_geometry_to_objects:
            if message_type in {'v1', 'relational'}:
                ...
                if message_type in {'v2', 'non-relational'}:
                    ...
                    self.geometry_to_object_iim = IIM(2*hidden_size, hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                    if message_segment:
                        self.geometry_to_object_segment_iim = IIM(hidden_size, hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                if message_aggregation in {'att', 'attention'}:
                    if attention_style in {'v4', 'general'}:
        ...
        # # Semantic(s) to Human
        # if message_semantic_to_human:
        #     if message_type in {'v1', 'relational'}:
        #         self.human_semantic_pairwise_relation_mlp = build_mlp([4 * hidden_size, hidden_size], ['relu'], bias=bias)
        #         self.human_semantic_full_relation_mlp = build_mlp([hidden_size, hidden_size], ['relu'], bias=bias)
        #         if message_segment:
        #             self.human_semantic_segment_pairwise_relation_mlp = build_mlp([2 * hidden_size, hidden_size], ['relu'],
        #                                                                        bias=bias)
        #             self.human_semantic_segment_full_relation_mlp = build_mlp([hidden_size, hidden_size], ['relu'],
        #                                                                    bias=bias)
        #     else:  # v2 or non-relational
        #         if message_type in {'v2', 'non-relational'}:
        #             if message_granularity in {'v1', 'generic'}:
        #                 self.semantic_to_human_message_mlp = build_mlp([2 * hidden_size, hidden_size], ['relu'], bias=bias)
        #                 if message_segment:
        #                     self.semantic_to_human_segment_message_mlp = build_mlp([hidden_size, hidden_size], ['relu'],
        #                                                                         bias=bias)
        #             else:  # v2 or specific
        #                 self.semantic_to_human_message_mlp = build_mlp([4 * hidden_size, hidden_size], ['relu'], bias=bias)
        #                 if message_segment:
        #                     self.semantic_to_human_segment_message_mlp = build_mlp([2 * hidden_size, hidden_size], ['relu'],
        #                                                                         bias=bias)
        #         else:  # v3 or iim
        #             self.semantic_to_human_iim = IIM(2*hidden_size, hidden_size, hidden_size, self.iim_layer_num,
        #                                            ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
        #             if message_segment:
        #                 self.semantic_to_human_segment_iim = IIM(hidden_size, hidden_size, hidden_size, self.iim_layer_num,
        #                                            ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
        #         if message_aggregation in {'att', 'attention'}:
        #             if attention_style in {'v4', 'general'}:
        #                 self.semantic_to_human_message_att_mlp = nn.Bilinear(2 * hidden_size, 2 * hidden_size, 1,
        #                                                                    bias=bias)
        #                 if message_segment:
        #                     self.semantic_to_human_segment_message_att_mlp = nn.Bilinear(hidden_size, hidden_size, 1,
        #                                                                                bias=bias)
        #             else:
        #                 self.semantic_to_human_message_att_mlp = build_mlp([4 * hidden_size, 1], ['relu'], bias=bias)
        #                 if message_segment:
        #                     self.semantic_to_human_segment_message_att_mlp = build_mlp([2 * hidden_size, 1], ['relu'],
        #                                                                              bias=bias)
        # # Semantic(s) to Object
        # if message_semantic_to_objects:
        #     if message_type in {'v1', 'relational'}:
        #         self.object_semantic_pairwise_relation_mlp = build_mlp([4 * hidden_size, hidden_size], ['relu'], bias=bias)
        #         self.object_semantic_full_relation_mlp = build_mlp([hidden_size, hidden_size], ['relu'], bias=bias)
        #         if message_segment:
        #             self.object_semantic_segment_pairwise_relation_mlp = build_mlp([2 * hidden_size, hidden_size],
        #                                                                         ['relu'], bias=bias)
        #             self.object_semantic_segment_full_relation_mlp = build_mlp([hidden_size, hidden_size], ['relu'],
        #                                                                     bias=bias)
        #     else:  # v2 or non-relational
        #         if message_type in {'v2', 'non-relational'}:
        #             if message_granularity in {'v1', 'generic'}:
        #                 self.semantic_to_object_message_mlp = build_mlp([2 * hidden_size, hidden_size], ['relu'], bias=bias)
        #                 if message_segment:
        #                     self.semantic_to_object_segment_message_mlp = build_mlp([hidden_size, hidden_size], ['relu'],
        #                                                                         bias=bias)
        #             else:  # v2 or specific
        #                 self.semantic_to_object_message_mlp = build_mlp([4 * hidden_size, hidden_size], ['relu'], bias=bias)
        #                 if message_segment:
        #                     self.semantic_to_object_segment_message_mlp = build_mlp([2 * hidden_size, hidden_size], ['relu'],
        #                                                                         bias=bias)
        #         else:  # v3 or iim
        #             self.semantic_to_object_iim = IIM(2*hidden_size, hidden_size, hidden_size, self.iim_layer_num,
        #                                            ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
        #             if message_segment:
        #                 self.semantic_to_object_segment_iim = IIM(hidden_size, hidden_size, hidden_size, self.iim_layer_num,
        #                                            ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
        #         if message_aggregation in {'att', 'attention'}:
        #             if attention_style in {'v4', 'general'}:
        #                 self.semantic_to_object_message_att_mlp = nn.Bilinear(2 * hidden_size, 2 * hidden_size, 1,
        #                                                                     bias=bias)
        #                 if message_segment:
        #                     self.semantic_to_object_segment_message_att_mlp = nn.Bilinear(hidden_size, hidden_size, 1,
        #                                                                                 bias=bias)
        #             else:
        #                 self.semantic_to_object_message_att_mlp = build_mlp([4 * hidden_size, 1], ['relu'], bias=bias)
        #                 if message_segment:
        #                     self.semantic_to_object_segment_message_att_mlp = build_mlp([2 * hidden_size, 1], ['relu'],
        #                                                                               bias=bias)
    ...
    def forward(self, x_human, x_objects, objects_mask, human_segmentation=None, objects_segmentation=None,
                ...
                caption_feature = None, action_feature = None, 
        ...
        ### gcn ###
        bs, time_step = x_human.shape[:2]
        if x_human.shape[3] == 2124:                   # CAD120
            x_human, x_geometry = torch.split(x_human, [2048, 76], dim=-1)
            x_geometry = x_geometry.squeeze(2)
            x_geometry = x_geometry.view(bs, time_step, 76 // 4, 4)
            x_geometry_hh = x_geometry[:,:,:9]
            x_geometry_oo = x_geometry[:,:,9:]
            x_geometry_ho1 = x_geometry.permute(0, 3, 2, 1).contiguous()
            if self.ablation_GE:
                x_geometry = x_geometry.permute(0, 3, 2, 1).contiguous()
            x_geometry_hg = torch.mean(x_geometry_hh, dim=2).unsqueeze(2)
            x_geometry_og = torch.mean(x_geometry_oo.view(bs, time_step, 5, 2, 4), dim=3)

            x_geometry_hh = x_geometry_hh.permute(0, 3, 2, 1).contiguous()
            x_geometry_oo = x_geometry_oo.permute(0, 3, 2, 1).contiguous()
            x_geometry_global = torch.cat([x_geometry_hg, x_geometry_og], dim=2).permute(0, 3, 2, 1).contiguous()

        elif x_human.shape[3] == 2168:                 # Bimacs
            x_human, x_geometry = torch.split(x_human, [2048, 120], dim=-1)
            x_geometry = x_geometry[:, :, 0, :]
            x_geometry = x_geometry.view(bs, time_step, 120 // 4, 4)
            x_geometry_hh = x_geometry[:,:,:12]
            x_geometry_oo = x_geometry[:,:,12:]
            if self.ablation_GE:
                x_geometry = x_geometry.permute(0, 3, 2, 1).contiguous()
            x_geometry_ho1 = torch.cat([x_geometry_hh[:,:,:6], x_geometry_oo],dim=2).permute(0, 3, 2, 1).contiguous()
            x_geometry_ho2 = torch.cat([x_geometry_hh[:,:,6:], x_geometry_oo],dim=2).permute(0, 3, 2, 1).contiguous()
            x_geometry_hg = torch.mean(x_geometry_hh.view(bs, time_step, 2, 6, 4), dim=3)
            x_geometry_og = torch.mean(x_geometry_oo.view(bs, time_step, 9, 2, 4), dim=3)

            x_geometry_hh = x_geometry_hh.permute(0, 3, 2, 1).contiguous()
            x_geometry_oo = x_geometry_oo.permute(0, 3, 2, 1).contiguous()
            x_geometry_global = torch.cat([x_geometry_hg, x_geometry_og], dim=2).permute(0, 3, 2, 1).contiguous()

        elif x_human.shape[3] == 2136:                 # Bimacs
            x_human, x_geometry = torch.split(x_human, [2048, 88], dim=-1)
            x_geometry = x_geometry[:, :, 0, :]
            x_geometry = x_geometry.view(bs, time_step, 88 // 4, 4)
            x_geometry_hh = x_geometry[:,:,:12]
            x_geometry_oo = x_geometry[:,:,12:]
            if self.ablation_GE:
                x_geometry = x_geometry.permute(0, 3, 2, 1).contiguous()
            ...
            x_geometry = x_geometry[:, :, 0, :]
            x_geometry = x_geometry.view(bs, time_step, 104 // 4, 4)
            x_geometry_hh = x_geometry[:,:,:18]
            x_geometry_oo = x_geometry[:,:,18:]
            if self.ablation_GE:
                x_geometry = x_geometry.permute(0, 3, 2, 1).contiguous()
            x_geometry_ho1 = torch.cat([x_geometry_hh[:,:,:9], x_geometry_oo], dim=2).permute(0, 3, 2, 1).contiguous()      # bs, 4, p , t
            x_geometry_ho2 = torch.cat([x_geometry_hh[:,:,9:], x_geometry_oo], dim=2).permute(0, 3, 2, 1).contiguous()
            x_geometry_hg = torch.mean(x_geometry_hh.view(bs, time_step, 2, 9, 4), dim=3)
            x_geometry_og = torch.mean(x_geometry_oo.view(bs, time_step, 4, 2, 4), dim=3)

            x_geometry_hh = x_geometry_hh.permute(0, 3, 2, 1).contiguous()
            x_geometry_oo = x_geometry_oo.permute(0, 3, 2, 1).contiguous()
            x_geometry_global = torch.cat([x_geometry_hg, x_geometry_og], dim=2).permute(0, 3, 2, 1).contiguous()
        
        # del x_geometry, x_geometry_hg, x_geometry_og
        del x_geometry_hg, x_geometry_og
        torch.cuda.empty_cache()
        if self.ablation_GE:
            x_geometry = self._geometric_embed(x_geometry, self.geometry_embedding_gcn, self.geometry_embedding_mlp, bs, time_step, self.delta)
        
        else:
            # bs, t, vw = x_geometry.size()
            # x_geometry = x_geometry.view(bs, t, vw // 4, 4)
            x_geometry_ho1 = self._geometric_embed(x_geometry_ho1, self.geometry_embedding_gcn_ho1, self.geometry_embedding_mlp_ho1, bs, time_step, self.delta)
            x_geometry_oo = self._geometric_embed(x_geometry_oo, self.geometry_embedding_gcn_oo, self.geometry_embedding_mlp_oo, bs, time_step, self.delta)
            x_geometry_global = self._geometric_embed(x_geometry_global, self.geometry_embedding_gcn_global, self.geometry_embedding_mlp_global, bs, time_step, self.delta)
            x_geometry_hh = self._geometric_embed(x_geometry_hh, self.geometry_embedding_gcn_hh, self.geometry_embedding_mlp_hh, bs, time_step, self.delta)
            if self.human_num >1:
                x_geometry_ho2 = self._geometric_embed(x_geometry_ho2, self.geometry_embedding_gcn_ho2, self.geometry_embedding_mlp_ho2, bs, time_step, self.delta)
            
            # x_human, x_objects = cp.checkpoint(self.human_embedding_mlp, x_human, use_reentrant=False), cp.checkpoint(self.object_embedding_mlp, x_objects, use_reentrant=False)
        x_human, x_objects = self.human_embedding_mlp(x_human), self.object_embedding_mlp(x_objects)
        x_visual = []
        
        for h in range(self.human_num):
            for o in range(x_objects.shape[2]):
                x_visual.append(torch.cat([x_human[:,:,h], x_objects[:,:,o]], dim=-1))
        if self.human_num>1:
            for h1 in range(self.human_num):
                for h2 in range(self.human_num):
                    x_visual.append(torch.cat([x_human[:,:,h1], x_human[:,:,h2]], dim=-1))
        x_visual = torch.stack(x_visual, dim=-2)


        if self.ablation_GE:
            x_geometry_entity = x_geometry

        else:
            if self.human_num >1:
                x_geometry_ho = torch.cat([x_geometry_ho1, x_geometry_ho2], dim=2)
            else:
                x_geometry_ho = x_geometry_ho1
            x_geometry_oo = x_geometry_oo.repeat(1,1,x_objects.shape[2],1)
            x_human = x_human + self.geometry_ho_weight*x_geometry_ho
            x_objects = x_objects + self.geometry_oo_weight*x_geometry_oo

            x_geometry_entity = torch.cat([x_geometry_global, x_geometry_hh], dim=-1)
            x_geometry_entity = self.geometry_proj_mlp(x_geometry_entity)

        if self.caption_message:
            input_caption_feature = self.caption_proj(caption_feature).transpose(0,1) 
            input_action_feature = self.action_proj(action_feature).transpose(0,1)
            input_visual_feature = self.visual_proj(x_visual).permute(1,2,0,3).contiguous()
            caption_action_features = []
            caption_visual_features = []
            for t in range(time_step):
                caption_action_feature = self.caption_action_match(input_action_feature, input_caption_feature[t].unsqueeze(0))
            # caption_action_features = torch.stack(output, dim=0)
                caption_visual_feature = self.visual_action_match(caption_action_feature, input_visual_feature[t])
                caption_action_features.append(caption_action_feature)
                caption_visual_features.append(caption_visual_feature)
            caption_action_features = torch.stack(caption_action_features,dim=0).permute(2,0,1,3).contiguous()
            caption_visual_features = torch.stack(caption_visual_features,dim=0).permute(2,0,1,3).contiguous()
            
            caption_visual_features_re = self.caption_dim_redu_recognition(caption_visual_features).squeeze(-1).view(bs, time_step, -1, num_humans).transpose(-1,-2)
            caption_visual_features_pre = self.caption_dim_redu_prediction(caption_visual_features).squeeze(-1).view(bs, time_step, -1, num_humans).transpose(-1,-2)
            
            caption_visual_features_re = F.log_softmax(caption_visual_features_re, dim= -1).permute(0,3,1,2).contiguous()
            caption_visual_features_pre = F.log_softmax(caption_visual_features_pre, dim= -1).permute(0,3,1,2).contiguous()
            # caption_visual_features_re = F.leaky_relu(caption_visual_features_re).permute(0,3,1,2).contiguous()
            # caption_visual_features_pre = F.leaky_relu(caption_visual_features_pre).permute(0,3,1,2).contiguous()
            # if self.message_semantic_to_human or self.message_semantic_to_objects:
            #     x_semantic_entity = self.semantic_proj_mlp(caption_action_features)

        
        # print(caption_action_features_re)
        ...
        h_sf, h_sfr = self._process_frame_level_rnn(x_geometry_entity, self.geometry_bd_rnn, self.geometry_bd_embedding_mlp)
        # if self.message_semantic_to_human or self.message_semantic_to_objects:
        #     h_tf, h_tfr = self._process_frame_level_rnn(x_semantic_entity, self.semantic_bd_rnn, self.semantic_bd_embedding_mlp)
        ...
        for t in range(num_steps):
            ...
            for h in range(num_humans):
                ...
                if self.message_geometry_to_human:
                    ...
                    #except TypeError:
                    #    hs_dists = None
                    m_shth = self._geometry_to_human_message(x_geometry_entity[:, t], x_hfth, h_sf[:, t], h_hfth,
                                                                       hs_dists=None)#hs_dists)
                # m_thth = None
                # if self.message_semantic_to_human:
                #     m_thth = self._semantic_to_human_message(x_semantic_entity[:, t], x_hfth, h_tf[:,t], h_hfth)
                try:
            ...
            for k in range(num_objects):
                ...
                if self.message_geometry_to_objects:
                    ...
                    #except TypeError:
                    #    os_dists = None
                    m_sotk = self._geometry_to_object_message(x_geometry_entity[:, t], x_oftk, h_sf[:, t], h_oftk,
                                                            os_dists=None)#os_dists)
                    m_sotk = m_sotk * objects_mask[:, k:k + 1]
                # m_totk = None
                # if self.message_semantic_to_objects:
                #     m_totk = self._semantic_to_object_message(x_semantic_entity[:, t], x_oftk, h_tf[:, t], h_oftk)
                #     m_totk = m_totk * objects_mask[:, k:k + 1]
        ...
        if self.caption_message:
            # y_human_frame_recognition = self.human_frame_recognition_mlp(h_hfr).permute(0, 3, 1, 2).contiguous() + self.caption_action_fusion_weight*caption_visual_features_re
            # y_human_frame_prediction = self.human_frame_prediction_mlp(h_hfr).permute(0, 3, 1, 2).contiguous() + self.caption_action_fusion_weight*caption_visual_features_pre
            y_human_recognition = self.human_recognition_mlp(hx_hs).permute(0, 3, 1, 2).contiguous()  + F.sigmoid(self.caption_action_fusion_weight_recognition)*caption_visual_features_re
            y_human_prediction = self.human_prediction_mlp(hx_hs).permute(0, 3, 1, 2).contiguous() + F.sigmoid(self.caption_action_fusion_weight_prediction)*caption_visual_features_pre
        else:
        ...
        # print(y_human_frame_recognition)
        # print(y_human_prediction)
        # try:
    ...
    @staticmethod
    def _geometric_embed(x_geometry, geometric_embedding_gcn, geometric_embedding_mlp, batch, time, delta=True):
        # def geo_embed_fn(x):
        x_geometry = geometric_embedding_gcn(x_geometry)
        x_geometry = x_geometry.unsqueeze(2)
        if delta:
            x_geometry = x_geometry.view(batch, time, 1, x_geometry.shape[-2] * x_geometry.shape[-1])
        else:
            x_geometry = x_geometry.view(batch, time, 1, x_geometry.shape[1] * x_geometry.shape[3])
        x_geometry = geometric_embedding_mlp(x_geometry)
        # x_geometry = cp.checkpoint(geometric_embedding_mlp, x_geometry, use_reentrant=False)
        return x_geometry
    ...
    def _process_frame_level_rnn(x, rnn, embedding_mlp):
        ...
        # h_fr = cp.checkpoint(embedd_rnn_fn, x, use_reentrant=False)
        # h_f = cp.checkpoint(embedding_mlp,h_fr, use_reentrant=False)
    ...
    def _humans_to_human_message(self, x_hft, h_hft, h, hh_dists=None):
        ...
        if self.message_type in {'v1', 'relational'}:
            ...
            if self.message_type in {'v2', 'non-relational'}:
                ...
                m_hhth = compute_information_interaction_message(receiver, senders, senders_mask,
                                                                 iim_module=self.humans_to_human_iim)
    ...
    def _humans_to_human_segment_message(self, hx_hs, h, batch_size, dtype, device, direction: str, hh_dists=None):
        ...
        if self.message_type in {'v1', 'relational'}:
            ...
            if self.message_type in {'v2', 'non-relational'}:
                ...
                mg_hhth = compute_information_interaction_message(receiver, senders, senders_mask,
                                                                 iim_module=self.humans_to_human_segment_iim)
            if self.message_aggregation in {'mp', 'mean_pooling'}:
                num_real_senders = torch.sum(senders_mask, dim=1, keepdim=True)
                num_real_senders = torch.clamp(num_real_senders, min=1.0)
    ...
    def _humans_to_object_message(self, x_hft, x_oftk, h_hft, h_oftk, oh_dists=None):
        ...
        if self.message_type in {'v1', 'relational'}:
            ...
            if self.message_type in {'v2', 'non-relational'}:
                ...
                hok_message = compute_information_interaction_message(receiver, senders, senders_mask,
                                                                 iim_module=self.humans_to_object_iim)
            if self.message_aggregation in {'mp', 'mean_pooling'}:
                num_real_senders = torch.sum(senders_mask, dim=1, keepdim=True)
                num_real_senders = torch.clamp(num_real_senders, min=1.0)
    ...
    def _humans_to_object_segment_message(self, hx_hok, hx_hs, batch_size, dtype, device, direction: str,
        ...
        if self.message_type in {'v1', 'relational'}:
            ...
            if self.message_type in {'v2', 'non-relational'}:
                ...
                mg_hot = compute_information_interaction_message(receiver, senders, senders_mask,
                                                                 iim_module=self.humans_to_object_segment_iim)
            if self.message_aggregation in {'mp', 'mean_pooling'}:
                num_real_senders = torch.sum(senders_mask, dim=1, keepdim=True)
                num_real_senders = torch.clamp(num_real_senders, min=1.0)
    ...
    def _objects_to_human_message(self, x_hfth, x_oft, h_hfth, h_oft, objects_mask, ho_dists=None):
        ...
        if self.message_type in {'v1', 'relational'}:
            ...
            if self.message_type in {'v2', 'non-relational'}:
                ...
                m_oht = compute_information_interaction_message(receiver, senders, senders_mask,
                                                                 iim_module=self.objects_to_human_iim)
            if self.message_aggregation in {'mp', 'mean_pooling'}:
                num_real_senders = torch.sum(senders_mask, dim=1, keepdim=True)
                num_real_senders = torch.clamp(num_real_senders, min=1.0)
    ...
    def _objects_to_human_segment_message(self, hx_hsh, hx_os, objects_mask, direction: str, ho_dists=None):
        ...
        if self.message_type in {'v1', 'relational'}:
            ...
            if self.message_type in {'v2', 'non-relational'}:
                ...
                mg_oht = compute_information_interaction_message(receiver, senders, senders_mask,
                                                                iim_module=self.objects_to_human_segment_iim)
            if self.message_aggregation in {'mp', 'mean_pooling'}:
                num_real_senders = torch.sum(senders_mask, dim=1, keepdim=True)
                num_real_senders = torch.clamp(num_real_senders, min=1.0)
    ...
    def _objects_to_object_message(self, x_oft, h_oft, k, objects_mask, oo_dists=None):
        ...
        if self.message_type in {'v1', 'relational'}:
            ...
            if self.message_type in {'v2', 'non-relational'}:
                ...
                m_ootk = compute_information_interaction_message(receiver, senders, senders_mask,
                                                                 iim_module=self.objects_to_object_iim)
            if self.message_aggregation in {'mp', 'mean_pooling'}:
                num_real_senders = torch.sum(senders_mask, dim=1, keepdim=True)
                num_real_senders = torch.clamp(num_real_senders, min=1.0)
    ...
    def _objects_to_object_segment_message(self, hx_os, objects_mask, k, direction: str, oo_dists=None):
        ...
        if self.message_type in {'v1', 'relational'}:
            ...
            if self.message_type in {'v2', 'non-relational'}:
                ...
                mg_oot = compute_information_interaction_message(receiver, senders, senders_mask,
                                                                iim_module=self.objects_to_object_segment_iim)
            if self.message_aggregation in {'mp', 'mean_pooling'}:
                num_real_senders = torch.sum(senders_mask, dim=1, keepdim=True)
                num_real_senders = torch.clamp(num_real_senders, min=1.0)
    ...
    def _geometry_to_object_message(self, x_sft, x_oftk, h_sft, h_oftk, os_dists=None):
        ...
        if self.message_type in {'v1', 'relational'}:
            ...
            if self.message_type in {'v2', 'non-relational'}:
                ...
                hok_message = compute_information_interaction_message(receiver, senders, senders_mask,
                                                                 iim_module=self.geometry_to_object_iim)
            if self.message_aggregation in {'mp', 'mean_pooling'}:
                num_real_senders = torch.sum(senders_mask, dim=1, keepdim=True)
                num_real_senders = torch.clamp(num_real_senders, min=1.0)
                hok_message = torch.sum(hok_message, dim=1) / num_real_senders
    ...
    def _geometry_to_human_message(self, x_sft, x_hftk, h_sft, h_hftk, hs_dists=None):
        ...
        if self.message_type in {'v1', 'relational'}:
            ...
            if self.message_type in {'v2', 'non-relational'}:
                ...
                hok_message = compute_information_interaction_message(receiver, senders, senders_mask,
                                                                 iim_module=self.geometry_to_human_iim)
            if self.message_aggregation in {'mp', 'mean_pooling'}:
                num_real_senders = torch.sum(senders_mask, dim=1, keepdim=True)
                num_real_senders = torch.clamp(num_real_senders, min=1.0)
                hok_message = torch.sum(hok_message, dim=1) / num_real_senders
    ...
    def _semantic_to_object_message(self, x_tft, x_oftk, h_tft, h_oftk):
        """Compute human to an object message.
        ...
        """
        receiver = torch.cat([x_oftk, h_oftk], dim=-1)
        senders = torch.cat([x_tft, h_tft], dim=-1)
        senders_mask = torch.ones(senders.size()[:2], dtype=senders.dtype, device=senders.device)
        if self.message_type in {'v1', 'relational'}:
            hok_message = compute_relational_message(receiver, senders, senders_mask,
                                                     f=self.object_geometry_full_relation_mlp,
    ...
    def _semantic_to_human_message(self, x_tft, x_hftk, h_tft, h_hftk):
        """Compute human to an object message.
...
def compute_information_interaction_message(receiver, senders, senders_mask, iim_module):
    iim_message = []
    num_senders = senders.size(1)
    for s in range(num_senders):
        sender = senders[:, s]
        # receiver_s = receiver
        # sender = torch.cat([receiver, sender], dim=-1)
        # receiver_s = torch.cat([receiver_s, receiver_s], dim=-1)
        m_sr = iim_module(sender) * senders_mask[:, s:s + 1]
        iim_message.append(m_sr)
    iim_message = torch.stack(iim_message, dim=1)
    return iim_message
