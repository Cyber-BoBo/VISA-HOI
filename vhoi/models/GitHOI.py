...
class GitHOI(nn.Module):
    def __init__(self, input_size: tuple, num_classes: tuple, hidden_size: int = 128,
        ...
        self.message_semantic_to_objects = message_semantic_to_objects
        self.message_semantic_to_human = message_semantic_to_human
        ...
        if message_humans_to_human:
            if message_type in {'v1', 'relational'}:
                ...
                if message_type in {'v2', 'non-relational'}:
                    ...
                    self.humans_to_human_iim = IIM(4*hidden_size, 2*hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                    if message_segment:
                        self.humans_to_human_segment_iim = IIM(2*hidden_size, hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
        ...
        if message_human_to_objects:
            if message_type in {'v1', 'relational'}:
                ...
                if message_type in {'v2', 'non-relational'}:
                    ...
                    self.humans_to_object_iim = IIM(4*hidden_size, 2*hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                    if message_segment:
                        self.humans_to_object_segment_iim = IIM(2*hidden_size, hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                if message_aggregation in {'att', 'attention'}:
                    if attention_style in {'v4', 'general'}:
        ...
        if message_objects_to_human:
            if message_type in {'v1', 'relational'}:
                ...
                if message_type in {'v2', 'non-relational'}:
                    ...
                    self.objects_to_human_iim = IIM(4*hidden_size, 2*hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                    if message_segment:
                        self.objects_to_human_segment_iim = IIM(2*hidden_size, hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                if message_aggregation in {'att', 'attention'}:
                    if attention_style in {'v4', 'general'}:
        ...
        if message_objects_to_object:
            if message_type in {'v1', 'relational'}:
                ...
                if message_type in {'v2', 'non-relational'}:
                    ...
                    self.objects_to_object_iim = IIM(4*hidden_size,2*hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                    if message_segment:
                        self.objects_to_object_segment_iim = IIM(2*hidden_size, hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                if message_aggregation in {'att', 'attention'}:
                    if attention_style in {'v4', 'general'}:
        ...
        if message_geometry_to_human:
            if message_type in {'v1', 'relational'}:
                ...
                if message_type in {'v2', 'non-relational'}:
                    ...
                    self.geometry_to_human_iim = IIM(4*hidden_size, 2*hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                    if message_segment:
                        self.geometry_to_human_segment_iim = IIM(2*hidden_size, hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                if message_aggregation in {'att', 'attention'}:
                    if attention_style in {'v4', 'general'}:
        ...
        if message_geometry_to_objects:
            if message_type in {'v1', 'relational'}:
                ...
                if message_type in {'v2', 'non-relational'}:
                    ...
                    self.geometry_to_object_iim = IIM(4*hidden_size, 2*hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                    if message_segment:
                        self.geometry_to_object_segment_iim = IIM(2*hidden_size, hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                if message_aggregation in {'att', 'attention'}:
                    if attention_style in {'v4', 'general'}:
        ...
        if message_semantic_to_human:
            if message_type in {'v1', 'relational'}:
                ...
                if message_type in {'v2', 'non-relational'}:
                    ...
                    self.semantic_to_human_iim = IIM(4*hidden_size, 2*hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                    if message_segment:
                        self.semantic_to_human_segment_iim = IIM(2*hidden_size, hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                if message_aggregation in {'att', 'attention'}:
                    if attention_style in {'v4', 'general'}:
        ...
        if message_semantic_to_objects:
            if message_type in {'v1', 'relational'}:
                ...
                if message_type in {'v2', 'non-relational'}:
                    ...
                    self.semantic_to_object_iim = IIM(4*hidden_size, 2*hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                    if message_segment:
                        self.semantic_to_object_segment_iim = IIM(2*hidden_size, hidden_size, hidden_size, self.iim_layer_num,
                                                   ['relu'] * self.iim_layer_num, num_heads=iim_num_heads, bias=bias)
                if message_aggregation in {'att', 'attention'}:
                    if attention_style in {'v4', 'general'}:
    ...
    def forward(self, x_human, x_objects, objects_mask, human_segmentation=None, objects_segmentation=None,
        ...
        if x_human.shape[3] == 2124:                   # CAD120
            ...
            x_geometry_ho1 = torch.cat([x_geometry_hh[:,:,:9], x_geometry_oo], dim=2).permute(0, 3, 2, 1).contiguous()
        ...
        x_geometry_ho1 = self._geometric_embed(x_geometry_ho1, self.geometry_embedding_gcn_ho1, self.geometry_embedding_mlp_ho1, bs, time_step)
        x_geometry_oo = self._geometric_embed(x_geometry_oo, self.geometry_embedding_gcn_oo, self.geometry_embedding_mlp_oo, bs, time_step)
        x_geometry_global = self._geometric_embed(x_geometry_global, self.geometry_embedding_gcn_global, self.geometry_embedding_mlp_global, bs, time_step)
        x_geometry_hh = self._geometric_embed(x_geometry_hh, self.geometry_embedding_gcn_hh, self.geometry_embedding_mlp_hh, bs, time_step)
        if self.human_num >1:
            x_geometry_ho2 = self._geometric_embed(x_geometry_ho2, self.geometry_embedding_gcn_ho2, self.geometry_embedding_mlp_ho2, bs, time_step)
        ...
        if self.caption_message:
            ...
            # caption_action_features = torch.stack(output, dim=0)
                ...
                caption_action_features.append(caption_visual_feature)
                caption_visual_features.append(caption_action_feature)
        ...
        for t in range(num_steps):
            ...
            for h in range(num_humans):
                ...
                try:
                    ...
                    u_hsth, u_hsths = self._update_human_segment(x_hfth, h_hfth, m_hhth, m_ohth, m_shth, m_thth, x_tt)
                    if t == (num_steps - 1):
                        u_hsth[:] = 1.0
                ux_hs[h].append(u_hsth)
                ux_hss[h].append(u_hsths)
                x_hsth = cat_valid_tensors([h_hfth, m_hhth, m_ohth, m_shth, m_thth], dim=-1)
            ...
            for k in range(num_objects):
                ...
                try:
                    ...
                    u_ostk, u_ostks = self._update_object_segment(x_oftk, h_oftk, m_hotk, m_ootk, m_sotk, m_totk, u_hst, u_hsts, x_tt)
                    if t == (num_steps - 1):
                        u_ostk[:] = 1.0
                ux_os[k].append(u_ostk)
                ux_oss[k].append(u_ostks)
                x_ostk = cat_valid_tensors([h_oftk, m_hotk, m_sotk, m_ootk, m_totk], dim=-1)
        ...
        if self.caption_message:
            ...
            y_human_recognition = self.human_recognition_mlp(hx_hs).permute(0, 3, 1, 2).contiguous() + self.caption_action_fusion_weight*caption_visual_features_re
            y_human_prediction = self.human_prediction_mlp(hx_hs).permute(0, 3, 1, 2).contiguous() + self.caption_action_fusion_weight*caption_visual_features_pre
        else:
    ...
    @staticmethod
    def _geometric_embed(x_geometry, geometric_embedding_gcn, geometric_embedding_mlp, batch, time):
    ...
    def _update_human_segment(self, x_hfth, h_hfth, m_hhth, m_ohth, m_shth, m_thth, x_tt):
        ...
        """
        update_human_segment_input = cat_valid_tensors([x_hfth, h_hfth, m_hhth, m_ohth, m_shth, m_thth, x_tt], dim=-1)
    ...
    def _update_object_segment(self, x_oftk, h_oftk, m_hotk, m_ootk, m_sotk, m_totk, u_hst, u_hsts, x_tt):
        """Operation to update current human segment-level discrete state.
        ...
        if self.object_segment_update_strategy in {'same_as_human', 'sah'} and u_hst is not None and u_hsts is not None:
            ...
            update_object_segment_input = cat_valid_tensors([x_oftk, h_oftk, m_hotk, m_ootk, m_sotk, m_totk, x_tt], dim=-1)
