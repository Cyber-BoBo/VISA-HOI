...
def select_model_data_fetcher(model_name: str, model_input_type: str, **kwargs):
    model_to_data_fetcher = {
        ...
        '2G-GCN': partial(fetchers.gcn_fetcher, **kwargs),
        'GitHOI': partial(fetchers.git_fetcher, **kwargs),
        'TIGAhoi': partial(fetchers.git_fetcher, **kwargs),
    }
...
def select_model_data_feeder(model_name: str, model_input_type: str, **kwargs):
    model_to_data_forwarder = {
        ...
        '2G-GCN': partial(forwarders.gcn_forward, **kwargs),
        # 'GitHOI': partial(forwarders.git_forward, **kwargs),
        'GitHOI': partial(forwarders.git_forward, **kwargs),
        'TIGAhoi': partial(forwarders.git_forward, **kwargs),
    }
...
def determine_num_classes(model_name: str, model_input_type: str, dataset_name: str):
    if model_name in {'2G-GCN', 'GitHOI', 'TIGAhoi', 'bimanual_baseline', 'cad120_baseline'}:
        if dataset_name.lower() == 'bimanual':
            return 14, None
        elif dataset_name.lower() == 'mphoi':
            return 13, None
        elif dataset_name.lower() == 'bimanual_interactions':
            return 13, None
        else:
            return 10, 12
...
def load_unseen_training_data(data, model_name, model_input_type, batch_size: int = 8, val_fraction: float = 0.2,
                       seed: int = 42, debug: bool = False, sigma: float = 0.0):
    data_path, data_path_zarr = data.path, data.path_zarr
    unseen_radio = data.unseen_radio
    train_task_id = data.unseen_task
    scaling_strategy = data.scaling_strategy        # 缩放策略
    downsampling = data.downsampling                # 降采样

    if 'MPHOI' in data_path:
        data_path_obbs_zarr = data.path_obb_zarr
        data_path_hbbs_zarr = data.path_hbb_zarr
        data_path_hps_zarr = data.path_hps_zarr
        data_path_action_zarr = data.path_action_zarr
        data_path_caption_zarr = data.path_caption_zarr
        data_path_unseen_train_test_split = data.path_unseen_train_test_split
        train_loader, data_info, scalers = \
            mphoi.load_unseen_training_data(data_path, data_path_zarr, data_path_obbs_zarr, data_path_hbbs_zarr, data_path_hps_zarr,
                                     data_path_action_zarr, data_path_caption_zarr, data_path_unseen_train_test_split, unseen_radio,
                                     model_name, model_input_type, train_task_id=train_task_id, batch_size=batch_size, 
                                     val_fraction=val_fraction, seed=seed, scaling_strategy=scaling_strategy, 
                                     sigma=sigma, downsampling=downsampling)
    else:
        print('Only MPHOI-72 is supported now.')
    
    return train_loader, data_info, scalers

def load_unseen_testing_data(data, model_name: str, model_input_type: str, batch_size: int, scalers: Optional[dict] = None):
    data_path, data_path_zarr = data.path, data.path_zarr
    unseen_radio = data.unseen_radio
    train_task_id = data.unseen_task
    ...
    data_path_action_zarr = data.get('path_action_zarr',default_value=None)
    data_path_caption_zarr = data.get('path_caption_zarr', default_value=None)
    data_path_unseen_train_test_split = data.get('path_unseen_train_test_split', default_value=None)
    ...
    test_loader, data_info, segmentations, test_ids = \
        mphoi.load_unseen_testing_data(data_path, data_path_zarr, data_path_obbs_zarr, data_path_hbbs_zarr, data_path_hps_zarr,
                                data_path_action_zarr, data_path_caption_zarr, data_path_unseen_train_test_split, unseen_radio,
                                model_name, model_input_type, train_task_id=train_task_id, batch_size=batch_size, 
...
def load_training_data(data, model_name, model_input_type, batch_size: int = 8, val_fraction: float = 0.2,
                       seed: int = 42, debug: bool = False, sigma: float = 0.0):
    data_path, data_path_zarr = data.path, data.path_zarr
    test_subject_id = data.cross_validation_test_subject        # 交叉验证测试数据
    scaling_strategy = data.scaling_strategy        # 缩放策略
    downsampling = data.downsampling                # 降采样
    ...
    if 'BimanualActions' in data_path:
        ...
        train_loader, val_loader, data_info, scalers = \
            bimanual.load_training_data(data_path, data_path_zarr, data_path_bbs_zarr, data_path_hps_zarr,
                                        data_path_action_zarr, data_path_caption_zarr,
                                        model_name, model_input_type,
                                        test_subject_id=test_subject_id, video_id_to_video_fps=video_id_to_video_fps,
                                        batch_size=batch_size, val_fraction=val_fraction, seed=seed,
                                        scaling_strategy=scaling_strategy, sigma=sigma, downsampling=downsampling)
    elif 'BimanualInteractions' in data_path:
        data_path_obbs_zarr = data.path_obb_zarr
        data_path_hbbs_zarr = data.path_hbb_zarr
        data_path_hps_zarr = data.path_hp_zarr
        data_path_action_zarr = data.path_action_zarr
        data_path_caption_zarr = data.path_caption_zarr
        train_loader, val_loader, data_info, scalers = \
            bimanual_interactions.load_training_data(data_path, data_path_zarr, data_path_obbs_zarr, data_path_hbbs_zarr, data_path_hps_zarr,
                                                     data_path_action_zarr, data_path_caption_zarr,
        ...
        data_path_action_zarr = data.path_action_zarr
        data_path_caption_zarr = data.path_caption_zarr
        data_path_similarity_zarr = data.path_similarity_zarr
        train_loader, val_loader, data_info, scalers = \
            mphoi.load_training_data(data_path, data_path_zarr, data_path_obbs_zarr, data_path_hbbs_zarr, data_path_hps_zarr,
                                     data_path_action_zarr, data_path_caption_zarr,
                                     model_name, model_input_type, test_subject_id=test_subject_id, batch_size=batch_size, 
                                     val_fraction=val_fraction, seed=seed, scaling_strategy=scaling_strategy, 
                                     sigma=sigma, downsampling=downsampling)
        ...
        train_loader, val_loader, data_info, scalers = \
            cad120.load_training_data(data_path, model_name, model_input_type,
                                      test_subject_id=test_subject_id,
                                      video_id_to_subject_id=video_id_to_subject_id,
                                      data_path_zarr=data_path_zarr,
                                      data_path_action_zarr=data_path_action_zarr, data_path_caption_zarr=data_path_caption_zarr,
...
def load_testing_data(data, model_name: str, model_input_type: str, batch_size: int, scalers: Optional[dict] = None):
    ...
    if 'BimanualActions' in data_path:
        ...
        test_loader, data_info, segmentations, test_ids = \
            bimanual.load_testing_data(data_path, data_path_zarr, data_path_bbs_zarr, data_path_hps_zarr,
                                       data_path_action_zarr, data_path_caption_zarr,
                                       model_name, model_input_type,
                                       test_subject_id=test_subject_id, video_id_to_video_fps=video_id_to_video_fps,
        ...
        if data_path_obbs_zarr is None:
            data_path_obbs_zarr = os.path.join(os.path.dirname(data_path_zarr), 'object_bounding_boxes.zarr')
        if data_path_hbbs_zarr is None:
            data_path_hbbs_zarr = os.path.join(os.path.dirname(data_path_zarr), 'hand_bounding_boxes.zarr')
        if data_path_hps_zarr is None:
            data_path_hps_zarr = os.path.join(os.path.dirname(data_path_zarr), 'human_pose_alphapose.zarr')
        if data_path_action_zarr is None:
            data_path_action_zarr = os.path.join(os.path.dirname(data_path_zarr), 'action_clip_text.zarr')
        if data_path_caption_zarr is None:
            data_path_caption_zarr = os.path.join(os.path.dirname(data_path_zarr), 'caption_clip_text.zarr')
        test_loader, data_info, segmentations, test_ids = \
            bimanual_interactions.load_testing_data(data_path, data_path_zarr, data_path_obbs_zarr, data_path_hbbs_zarr, data_path_hps_zarr,
                                                    data_path_action_zarr, data_path_caption_zarr,
                                                    model_name, model_input_type, test_subject_id=test_subject_id, batch_size=batch_size,
                                                    scalers=scalers, downsampling=downsampling)
    elif 'MPHOI' in data_path:
        ...
        if data_path_hps_zarr is None:
            data_path_hps_zarr = os.path.join(os.path.dirname(data_path_zarr), 'human_pose.zarr')
        test_loader, data_info, segmentations, test_ids = \
            mphoi.load_testing_data(data_path, data_path_zarr, data_path_obbs_zarr, data_path_hbbs_zarr, data_path_hps_zarr,
                                    data_path_action_zarr, data_path_caption_zarr,
                                    model_name, model_input_type, test_subject_id=test_subject_id, batch_size=batch_size, 
                                    scalers=scalers, downsampling=downsampling)
    else:  # CAD-120
        ...
        video_id_to_subject_id = read_dictionary(data.video_id_to_subject_id)
        test_loader, data_info, segmentations, test_ids = \
            cad120.load_testing_data(data_path, model_name, model_input_type,
                                     test_subject_id=test_subject_id, video_id_to_subject_id=video_id_to_subject_id,
                                     data_path_zarr=data_path_zarr,
                                     data_path_action_zarr=data_path_action_zarr, data_path_caption_zarr=data_path_caption_zarr,
                                     batch_size=batch_size, scalers=scalers,
...
def create_data_loader(x, y, model_name: str, batch_size: int, shuffle: bool,
                       scaling_strategy: Optional[str] = None, scalers: Optional[dict] = None):
...
def maybe_scale_input_tensors(x: list, model_name: str, scaling_strategy: Optional[str] = None,
    ...
    if there_are_no_scalers:
        ...
        if scaling_strategy is None:
            return x, scalers
    if model_name in {'GitHOI', '2G-GCN', 'TIGAhoi', 'bimanual_baseline', 'cad120_baseline'}:
...
def assemble_segmentations(data, model_name: str, dataset_name: str):
    segmentations = None
    if model_name in{'2G-GCN', 'GitHOI', 'TIGAhoi'}:
...
def assemble_cad120_segmentations_from_frame_level_features(data):
    segmentations = []
    for _, _, _, _, _, video_data, _, _ in data:
...
def assemble_caption_features(data):
    caption_features = []
    for _, _, _, _, _, _, _, _, _, caption,_  in data:
        caption_features.append(caption)
    return np.stack(caption_features, axis=0)
...
class mphoi:
    """MPHOI-72"""
    def load_unseen_training_data(data_path, data_path_zarr, data_path_obbs_zarr, data_path_hbbs_zarr, data_path_hps_zarr,
                           data_path_action_zarr, data_path_caption_zarr, data_path_unseen_train_test_split, unseen_radio,
                           model_name: str, model_input_type: str, train_task_id: str, batch_size: int = 8, val_fraction: float = 0.2, seed: int = 42, 
                                scaling_strategy=None, sigma: float = 0.0, downsampling: int = 1):
        with open(data_path, mode='rb') as f:   # 以2进制方式读取
            data = json.load(f)
        with open(data_path_unseen_train_test_split, mode= 'rb') as f:
        ...
        root_action_text = zarr.open(data_path_action_zarr, mode='r')
        root_caption = zarr.open(data_path_caption_zarr, mode='r')
        training_data = []
        for video_id, human_ground_truth in data.items():
            subject_id, task, take = video_id.split(sep='-')
            if video_id not in data_split['train_data'][str(unseen_radio)[-2:]][train_task_id]:
                continue
            ...
            Human2_bbs = root_hbbs[video_id]['Human2'][:]       # human bounding boxes
            objects_bbs = root_obbs[video_id]['objects'][:]     # object bounding boxes
            Human1_hps = root_hps[video_id]['Human1'][:]        # human pose
            Human2_hps = root_hps[video_id]['Human2'][:]
            Caption = root_caption[video_id][:]
            training_data.append([Human1_features, Human2_features, object_features, human_ground_truth,
                                Human1_bbs, Human2_bbs, objects_bbs, Human1_hps, Human2_hps, Caption, root_action_text['action_feature'][:]])
        random.seed(seed)
        random.shuffle(training_data)
        train_x, train_y = mphoi.assemble_tensors(training_data, model_name, sigma=sigma, downsampling=downsampling, test_data=False)
        # val_x, val_y = mphoi.assemble_tensors(val_data, model_name, sigma=sigma, downsampling=downsampling, test_data=False)
        # else:
        #     x, y = bimanual.assemble_tensors(data, model_name, sigma=sigma, downsampling=downsampling, test_data=test_data)

        train_loader, scalers = create_data_loader(train_x, train_y, model_name,
                                                      batch_size=batch_size, shuffle=True,
                                                      scaling_strategy=scaling_strategy)
        # val_loader, _ = create_data_loader(val_x, val_y, model_name,
        #                                       batch_size=len(val_data), shuffle=False,
        #                                       scalers=scalers)
        input_size = input_size_from_data_loader(train_loader, model_name, model_input_type)
        data_info = {'input_size': input_size}
        return train_loader, data_info, scalers

    def load_unseen_testing_data(data_path, data_path_zarr, data_path_obbs_zarr, data_path_hbbs_zarr, data_path_hps_zarr,
                            data_path_action_zarr, data_path_caption_zarr, data_path_unseen_train_test_split, unseen_radio,
                            model_name: str, model_input_type: str, train_task_id: int, batch_size: int, 
        ...
        for video_id, human_ground_truth in data.items():
            subject_id, task, take = video_id.split(sep='-')
            if video_id not in data_split['test_data'][str(unseen_radio)[-2:]][train_task_id]:
                continue
            Human1_features = root[video_id]['Human1'][:]
            Human2_features = root[video_id]['Human2'][:]
            object_features = root[video_id]['objects'][:]
        ...
        test_x, test_y = mphoi.assemble_tensors(testing_data, model_name, downsampling=downsampling, test_data=True)
        test_loader, _ = create_data_loader(test_x, test_y, model_name,
                                            batch_size=batch_size, shuffle=False,
                                            scalers=scalers)
        segmentations = assemble_segmentations(testing_data, model_name, dataset_name='mphoi')
        input_size = input_size_from_data_loader(test_loader, model_name, model_input_type)
        data_info = {'input_size': input_size}
        return test_loader, data_info, segmentations, test_ids


    def load_training_data(data_path, data_path_zarr, data_path_obbs_zarr, data_path_hbbs_zarr, data_path_hps_zarr,
                           data_path_action_zarr, data_path_caption_zarr,
                           model_name: str, model_input_type: str, test_subject_id: int, batch_size: int = 8, val_fraction: float = 0.2, seed: int = 42, 
                                scaling_strategy=None, sigma: float = 0.0, downsampling: int = 1):
        with open(data_path, mode='rb') as f:   # 以2进制方式读取
            data = json.load(f)
        root = zarr.open(data_path_zarr, mode='r')
        ...
        for video_id, human_ground_truth in data.items():
            ...
            Human2_features = root[video_id]['Human2'][:]
            object_features = root[video_id]['objects'][:]
            if data_path_hbbs_zarr.split('.')[-2].endswith('rtdetr'):
                Human1_bbs = root_hbbs[video_id]['Human1'][:].squeeze(1)
                Human2_bbs = root_hbbs[video_id]['Human2'][:].squeeze(1)       # human bounding boxes
            else:
                Human1_bbs = root_hbbs[video_id]['Human1'][:]
                Human2_bbs = root_hbbs[video_id]['Human2'][:]
    ...
    def load_testing_data(data_path, data_path_zarr, data_path_obbs_zarr, data_path_hbbs_zarr, data_path_hps_zarr,
                            data_path_action_zarr, data_path_caption_zarr,
    ...
    def assemble_tensors(data: list, model_name: str, sigma: float = 0.0, downsampling: int = 1,
                              test_data: bool = False):
        xs, ys = mphoi.assemble_frame_level_recurrent_human(data, downsampling=downsampling, test_data=test_data)
        xs_objects = mphoi.assemble_frame_level_recurrent_objects(data, downsampling=downsampling)
        if model_name in {'GitHOI', '2G-GCN', 'TIGAhoi'}:
            if sigma:
                ys[2] = ignore_last_step_end_flag_general(ys[2])
            ys[2] = smooth_segmentation(ys[2], sigma)
            ys_budget = ys[2]
            xs_hh_dists = mphoi.assemble_human_human_distances(data, downsampling=downsampling)
            xs_ho_dists = mphoi.assemble_human_object_distances(data, downsampling=downsampling)
            xs_oo_dists = mphoi.assemble_object_object_distances(data, downsampling=downsampling)
            action_text = mphoi.assemble_text_features(data)
            caption = mphoi.assemble_caption_features(data, downsampling=downsampling)
            xs_steps = assemble_num_steps(data, downsampling=downsampling)
            xs = xs[:1] + xs_objects + xs[1:] + [xs_hh_dists, xs_ho_dists, xs_oo_dists, xs_steps] + [caption] +  [action_text]
    ...
    def assemble_frame_level_recurrent_human(data, downsampling: int = 1, test_data: bool = False):
        ...
        for Human1, Human2, _, _, _, _, objects_bounding_box, Human1_pose, Human2_pose, caption, action in data:
            # human feature
        ...
        if xs_h1p[0].shape[1] == 32:
            keypoints = [1, 2, 4, 6, 7, 11, 13, 14, 27] # upper body keypoints
        else:
            keypoints = [0, 3, 4, 5, 6, 9, 10, 11, 12]
        ...
        for m, (_, _, _, video_hands_ground_truth, _, _, _, _, _,_,_) in enumerate(data):
            # Human1
    ...
    def assemble_frame_level_recurrent_objects(data, downsampling: int = 1):
        xs_objects = []
        max_len, max_len_downsampled, max_num_objects = 0, 0, 0
        for _, _, objects, _, _, _, _, _, _,_,_ in data:
        ...
        for m, x_o in enumerate(xs_objects):
            ...
            valid_objects = ~np.all(np.isnan(x_o), axis=(0, 2))
            x_objects_mask[m, :x_o.shape[1]] = valid_objects.astype(np.float32)
        xs = [x_objects, x_objects_mask]
        return xs
    
    def assemble_human_human_distances(data, downsampling: int = 1):
        mphoi_dims = np.array([3840, 2160], dtype=np.float32)
        max_len, max_num_humans = 0, 2
        all_dists = []
        for _, _, _, _, h1_bb, h2_bb, _, _, _, _, _ in data:
    ...
    def assemble_human_object_distances(data, downsampling: int = 1):
        ...
        h1_dists, h2_dists = [], []
        for _, _, _, _, h1_bb, h2_bb, obj_bbs, _, _ ,_, _ in data:
            h1_bb = h1_bb[downsampling - 1::downsampling]
            h1_centroids = compute_centroid(h1_bb) / mphoi_dims
            h2_bb = h2_bb[downsampling - 1::downsampling]
            h2_centroids = compute_centroid(h2_bb) / mphoi_dims
    ...
    def assemble_object_object_distances(data, downsampling: int = 1):
        ...
        all_dists = []
        for _, _, _, _, _, _, obj_bbs, _, _, _, _ in data:
            obj_bbs = obj_bbs[downsampling - 1::downsampling]
    ...
    def assemble_text_features(data):
        text_features = []
        for _, _, _, _, _, _, _, _, _, _, action in data:
            text_features.append(action)
        return np.stack(text_features, axis=0)

    def assemble_caption_features(data, downsampling: int = 1):
        caption_features = []
        # xs_objects = []
        max_len = 0
        for _, _, _, _, _, _, _, _, _, caption,_  in data:
            max_len = max(max_len, caption.shape[0])
            caption_features.append(caption)
        feature_size = caption_features[-1].shape[-1]
        x_caption = np.full([len(caption_features), max_len, feature_size],
                            fill_value=np.nan, dtype=np.float32)
        for m, x_c in enumerate(caption_features):
            x_caption[m, :x_c.shape[0], :] = x_c
        return x_caption



class cad120:
    def load_training_data(data_path, model_name, model_input_type, test_subject_id, video_id_to_subject_id,
                              data_path_zarr=None,
                              data_path_action_zarr=None, data_path_caption_zarr=None,
                              batch_size=8, val_fraction=0.2, seed=42,
                              scaling_strategy=None, sigma: float = 0.0, downsampling: int = 1):
        root = None
        ...
        with open(data_path, mode='rb') as f:
            data = pickle.load(f)
        if data_path_action_zarr is not None:
            root_action_text = zarr.open(data_path_action_zarr, mode='r')
        if data_path_caption_zarr is not None:
            root_caption = zarr.open(data_path_caption_zarr, mode='r')
        training_data = []
        ...
        for video_id, video_data in data.items():
            ...
            if root is not None:
                training_data_datum = [root[video_id + '/skeleton'][:], root[video_id + '/objects'][:],
                                    ...
                                    video_data, Caption, root_action_text['action_feature'][:]]
            else:
        ...
        #     val_data = val_data[:1]
        train_x, train_y = cad120.assemble_tensors(training_data, model_name, sigma=sigma, downsampling=downsampling, test_data=False)
        val_x, val_y = cad120.assemble_tensors(val_data, model_name, sigma=sigma, downsampling=downsampling, test_data=False)
    ...
    def load_testing_data(data_path, model_name, model_input_type, test_subject_id, video_id_to_subject_id,
                             data_path_zarr=None, 
                             data_path_action_zarr=None, data_path_caption_zarr=None,
                             batch_size=8, scalers=None, downsampling: int = 1):
        root = None
        if data_path_zarr is not None:
            root = zarr.open(data_path_zarr, mode='r')
        with open(data_path, mode='rb') as f:
        ...
        for video_id, video_data in data.items():
            ...
            test_ids.append(video_id)
        test_x, test_y = cad120.assemble_tensors(testing_data, model_name, downsampling=downsampling, test_data=True)
        test_loader, _ = create_data_loader(test_x, test_y, model_name,
                                                           batch_size=batch_size, shuffle=False,
                                                           scalers=scalers)
        segmentations = assemble_segmentations(testing_data, model_name, dataset_name='cad120')
        input_size = input_size_from_data_loader(test_loader, model_name, model_input_type)
        data_info = {'input_size': input_size}
        return test_loader, data_info, segmentations, test_ids
    ...
    def assemble_tensors(data: list, model_name: str, sigma: float = 0.0, downsampling: int = 1,
                     test_data: bool = False):
        if model_name in {'GitHOI', '2G-GCN', 'TIGAhoi', 'cad120_baseline'}:
            xs, ys = cad120.assemble_frame_level_recurrent_human(data, downsampling=downsampling, test_data=test_data)
            if model_name in{'GitHOI', '2G-GCN', 'TIGAhoi'}:
                if sigma:
                    ys[2] = ignore_last_step_end_flag(ys[2])
                ys[2] = smooth_segmentation(ys[2], sigma)
                ys_budget = ys[2]
                xs_objects, ys_objects = cad120.assemble_frame_level_recurrent_objects(data, downsampling=downsampling,
                                                                                test_data=test_data)
                # if sigma:
                ...
                xs_ho_dists = cad120.assemble_human_object_distances(data, downsampling=downsampling)
                xs_oo_dists = cad120.assemble_object_object_distances(data, downsampling=downsampling)
                action_text = cad120.assemble_text_features(data)
                caption = cad120.assemble_caption_features(data, downsampling=downsampling)
                ...
                #     ys[:2] + ys_objects[:2] +
                #     ys[:2] + ys_objects[:2]) 0 2 4 5 8 9
                ys = [ys_budget] + ys[2:]+ ys[:2]
                ys += ys[-2:]
                xs_steps = assemble_num_steps(data, downsampling=downsampling)
                xs += [xs_ho_dists, xs_oo_dists, xs_steps] + [caption] +  [action_text]
                xs = add_fake_dimension_to_human_tensors(xs, [0, 3])
                ys = add_fake_dimension_to_human_tensors(ys, [0, 1, 2, 3, 4, 5])
            ...
            raise ValueError(f'CAD-120 code not implemented for {model_name} yet.')
        return xs, ys
    ...
    def assemble_frame_level_recurrent_human(data, downsampling: int = 1, test_data: bool = False):
        ...
        x_obb= []
        max_len, max_len_downsampled = 0, 0
        for human_features, _, _, objects_bounding_box, skeleton_pose, _, _, _ in data:
        ...
        for m, (_, _, _, _, _, video_data, _, _) in enumerate(data):
    ...
    def assemble_frame_level_recurrent_objects(data, downsampling: int = 1, test_data: bool = False):
        ...
        for _, object_features, _, _, _, _, _, _ in data:
    ...
    def assemble_human_object_distances(data, downsampling: int = 1):
        ...
        max_len, max_num_objects = 0, 0
        all_dists = []
        for _, _, skeleton_bounding_box, objects_bounding_box, _, _, _, _ in data:
    ...
    def assemble_object_object_distances(data, downsampling: int = 1):
        max_len, max_num_objects = 0, 0
        all_dists = []
        for _, _, _, objects_bounding_box, _, _, _, _ in data:
            objects_bounding_box = objects_bounding_box[downsampling - 1::downsampling]
            objects_centroid = compute_centroid(objects_bounding_box)
    ...
    def assemble_text_features(data):
        text_features = []
        for  _, _, _, _, _, _, _, action in data:
            text_features.append(action)
        return np.stack(text_features, axis=0)
    ...
    def assemble_caption_features(data, downsampling: int = 1):
        ...
        # xs_objects = []
        max_len = 0
        for _, _, _, _, _, _, caption, _  in data:
            max_len = max(max_len, caption.shape[0])
            caption_features.append(caption[:,0])
        feature_size = caption_features[-1].shape[-1]
        x_caption = np.full([len(caption_features), max_len, feature_size],
                            fill_value=np.nan, dtype=np.float32)
        for m, x_c in enumerate(caption_features):
...
class bimanual:
    def load_training_data(data_path, data_path_zarr, data_path_bbs_zarr, data_path_hps_zarr, 
                           data_path_action_zarr, data_path_caption_zarr,
                           model_name: str, model_input_type: str,
                                test_subject_id: int, video_id_to_video_fps: dict, batch_size: int = 8,
                                val_fraction: float = 0.2, seed: int = 42, scaling_strategy=None,
        ...
        for video_id, hands_ground_truth in data.items():
            ...
            training_data.append([left_hand_features, right_hand_features, object_features, hands_ground_truth,
                                left_hand_bbs, right_hand_bbs, objects_bbs, left_hand_hps, right_hand_hps, Caption, root_action_text['action_feature'][:]])
        training_data, val_data = split_train_test(training_data, test_fraction=val_fraction, seed=seed)
        # if debug:
        #     training_data = training_data[:4]
        #     val_data = val_data[:1]

        train_x, train_y = bimanual.assemble_tensors(training_data, model_name, sigma=sigma, downsampling=downsampling, test_data=False)
        val_x, val_y = bimanual.assemble_tensors(val_data, model_name, sigma=sigma, downsampling=downsampling, test_data=False)
    ...
    def load_testing_data(data_path, data_path_zarr, data_path_bbs_zarr, data_path_hps_zarr, 
                          data_path_action_zarr, data_path_caption_zarr,
                          model_name: str, model_input_type: str,
        ...
        for video_id, hands_ground_truth in data.items():
            ...
            if video_fps == 15:  # Some videos were mistakenly collected at 15 FPS.
                ...
                hands_ground_truth['left_hand'] = np.repeat(hands_ground_truth['left_hand'], repeats=2, axis=0)
                hands_ground_truth['right_hand'] = np.repeat(hands_ground_truth['right_hand'], repeats=2, axis=0)
                Caption = np.repeat(Caption, repeats=2, axis=0)
                if video_id == 'subject_1-task_7_w_free_hard_drive-take_9':
                    Caption = np.concatenate([[Caption[0]],Caption[:]],axis=0)
            testing_data.append([left_hand_features, right_hand_features, object_features, hands_ground_truth,
                                left_hand_bbs, right_hand_bbs, objects_bbs, left_hand_hps, right_hand_hps, Caption, root_action_text['action_feature'][:]])
            test_ids.append(video_id)
        test_x, test_y = bimanual.assemble_tensors(testing_data, model_name, downsampling=downsampling, test_data=True)
        test_loader, _ = create_data_loader(test_x, test_y, model_name,
                                                           batch_size=batch_size, shuffle=False,
                                                           scalers=scalers)
        segmentations = assemble_segmentations(testing_data, model_name, dataset_name='bimanual')
        input_size = input_size_from_data_loader(test_loader, model_name, model_input_type)
        data_info = {'input_size': input_size}
        return test_loader, data_info, segmentations, test_ids
    ...
    def assemble_tensors(data: list, model_name: str, sigma: float = 0.0, downsampling: int = 1,
                              test_data: bool = False):
        xs, ys = bimanual.assemble_frame_level_recurrent_human(data, downsampling=downsampling, test_data=test_data)
        xs_objects = bimanual.assemble_frame_level_recurrent_objects(data, downsampling=downsampling)
        if model_name in {'GitHOI', '2G-GCN','TIGAhoi'}:
            if sigma:
                ys[2] = ignore_last_step_end_flag_general(ys[2])
            ys[2] = smooth_segmentation(ys[2], sigma)
            ys_budget = ys[2]
            xs_hh_dists = bimanual.assemble_human_human_distances(data, downsampling=downsampling)
            xs_ho_dists = bimanual.assemble_human_object_distances(data, downsampling=downsampling)
            xs_oo_dists = bimanual.assemble_object_object_distances(data, downsampling=downsampling)
            xs_steps = assemble_num_steps(data, downsampling=downsampling)
            action_text = bimanual.assemble_text_features(data)
            caption = bimanual.assemble_caption_features(data, downsampling=downsampling)
            xs = xs[:1] + xs_objects + xs[1:] + [xs_hh_dists, xs_ho_dists, xs_oo_dists, xs_steps] + [caption] + [action_text]
            ys = [ys_budget] + ys[2:] + ys[:2]
            ys += ys[-2:]
    ...
    def assemble_frame_level_recurrent_human(data, downsampling: int = 1, test_data: bool = False):
        ...
        x_obb= []
        max_len, max_len_downsampled = 0, 0
        for left_hand, right_hand, _, _, _, _, objects_bounding_box, left_hand_pose, right_hand_pose, _, _ in data:
            # hands feature
    ...
    def assemble_human_human_distances(data, downsampling: int = 1):
        ...
        max_len, max_num_humans = 0, 2
        all_dists = []
        for _, _, _, _, lh_bb, rh_bb, _, _, _,_,_ in data:
    ...
    def assemble_human_object_distances(data, downsampling: int = 1):
        ...
        max_len, max_num_humans, max_num_objects = 0, 2, 0
        lh_dists, rh_dists = [], []
        for _, _, _, _, lh_bb, rh_bb, obj_bbs, _, _,_,_ in data:
            lh_bb = lh_bb[downsampling - 1::downsampling]
            lh_centroids = compute_centroid(lh_bb) / bimanual_dims
            rh_bb = rh_bb[downsampling - 1::downsampling]
            rh_centroids = compute_centroid(rh_bb) / bimanual_dims
    ...
    def assemble_caption_features(data, downsampling: int = 1):
        ...
        x_caption = np.full([len(caption_features), max_len, feature_size],
                            fill_value=np.nan, dtype=np.float32)
        for m, x_c in enumerate(caption_features):
            # x_caption[m, :x_c.shape[0], :] = x_c[:,0,:]
            x_caption[m, :x_c.shape[0], :] = x_c
        return x_caption


class bimanual_interactions:
    @staticmethod
    def _filter_objects_never_appearing(
        object_features: np.ndarray,
        objects_bbs: np.ndarray,
        missing_ratio_threshold: float = 0.9,
        missing_or_static_max_objects: int = 9,
        static_valid_ratio_threshold: float = 0.5,
        static_mean_delta_threshold: float = 1.0,
        static_p95_delta_threshold: float = 3.0,
    ):
        if objects_bbs.ndim != 3:
            return object_features, objects_bbs
        missing_frames = np.any(np.isnan(objects_bbs), axis=2)
        missing_ratio = missing_frames.mean(axis=0)
        keep_mask = missing_ratio < missing_ratio_threshold
        if np.any(keep_mask):
            static_mask = np.zeros(objects_bbs.shape[1], dtype=bool)
            for i in np.where(keep_mask)[0]:
                valid = ~missing_frames[:, i]
                if valid.mean() < static_valid_ratio_threshold:
                    continue
                b = objects_bbs[valid, i]
                if b.shape[0] < 2:
                    continue
                deltas = np.abs(np.diff(b, axis=0)).reshape(-1)
                mean_delta = float(np.mean(deltas))
                p95_delta = float(np.percentile(deltas, 95))
                if mean_delta <= static_mean_delta_threshold and p95_delta <= static_p95_delta_threshold:
                    static_mask[i] = True
            keep_mask = keep_mask & (~static_mask)

        kept_indices = np.where(keep_mask)[0]
        if kept_indices.size == 0:
            return None, None
        if kept_indices.size > missing_or_static_max_objects:
            kept_indices = kept_indices[:missing_or_static_max_objects]
        object_features = object_features[:, kept_indices, :]
        objects_bbs = objects_bbs[:, kept_indices, :]
        # if object_features.shape[1] < missing_or_static_max_objects:
        #     pad_objects = missing_or_static_max_objects - object_features.shape[1]
        #     object_features = np.pad(
        #         object_features,
        #         pad_width=((0, 0), (0, pad_objects), (0, 0)),
        #         mode='constant',
    ...
    def load_training_data(data_path, data_path_zarr, data_path_obbs_zarr, data_path_hbbs_zarr, data_path_hps_zarr,
                           data_path_action_zarr, data_path_caption_zarr,
                           model_name: str, model_input_type: str, test_subject_id: int, batch_size: int = 8,
                           val_fraction: float = 0.2, seed: int = 42, scaling_strategy=None,
                           sigma: float = 0.0, downsampling: int = 1):
        with open(data_path, mode='rb') as f:
            data = json.load(f)
        ...
        training_data = []
        for video_id, hands_ground_truth in data.items():
            subject_id = int(video_id.split(sep='-')[0].replace('Subject', ''))
            if subject_id == test_subject_id:
                continue
            left_hand_features = root[video_id]['LeftHand'][:]
            right_hand_features = root[video_id]['RightHand'][:]
            object_features = root[video_id]['objects'][:]
            left_hand_bbs = root_hbbs[video_id]['LeftHand'][:]
            right_hand_bbs = root_hbbs[video_id]['RightHand'][:]
            objects_bbs = root_obbs[video_id]['objects'][:]
            object_features, objects_bbs = bimanual_interactions._filter_objects_never_appearing(object_features, objects_bbs)
            if object_features is None:
                continue
            left_hand_hps = root_hps[video_id]['LeftHand']['pose'][:]
            right_hand_hps = root_hps[video_id]['RightHand']['pose'][:]
            caption = root_caption[video_id]['clip_text'][:]
            if caption.shape[0] == root[video_id]['LeftHand'].shape[0]:
                caption = caption[:]
            hands_ground_truth = {
                'left_hand': hands_ground_truth['Lefthand'][0][:],
                'right_hand': hands_ground_truth['Righthand'][0][:],
            }
            training_data.append([
                left_hand_features, right_hand_features, object_features, hands_ground_truth,
                left_hand_bbs, right_hand_bbs, objects_bbs, left_hand_hps, right_hand_hps, caption,
                root_action_text['action_text_feature'][:],
            ])
        training_data, val_data = split_train_test(training_data, test_fraction=val_fraction, seed=seed)
        train_x, train_y = bimanual_interactions.assemble_tensors(training_data, model_name, sigma=sigma, downsampling=downsampling, test_data=False)
        val_x, val_y = bimanual_interactions.assemble_tensors(val_data, model_name, sigma=sigma, downsampling=downsampling, test_data=False)
        train_loader, scalers = create_data_loader(train_x, train_y, model_name,
                                                   batch_size=batch_size, shuffle=True,
                                                   scaling_strategy=scaling_strategy)
        val_loader, _ = create_data_loader(val_x, val_y, model_name,
    ...
    def load_testing_data(data_path, data_path_zarr, data_path_obbs_zarr, data_path_hbbs_zarr, data_path_hps_zarr,
        ...
        root_caption = zarr.open(data_path_caption_zarr, mode='r')
        testing_data, test_ids = [], []
        # frame_slice = slice(0, None, downsampling)
        for video_id, hands_ground_truth in data.items():
            subject_id = int(video_id.split(sep='-')[0].replace('Subject', ''))
            if subject_id != test_subject_id:
                continue
            ...
            test_ids.append(video_id)
        test_x, test_y = bimanual_interactions.assemble_tensors(testing_data, model_name, downsampling=1, test_data=True)
        test_loader, _ = create_data_loader(test_x, test_y, model_name,
                                            batch_size=batch_size, shuffle=False,
                                            scalers=scalers)
        segmentations = assemble_segmentations(testing_data, model_name, dataset_name='bimanual')
    ...
    def assemble_tensors(data: list, model_name: str, sigma: float = 0.0, downsampling: int = 1,
                         test_data: bool = False):
        xs, ys = bimanual_interactions.assemble_frame_level_recurrent_human(data, downsampling=downsampling, test_data=test_data)
        xs_objects = bimanual_interactions.assemble_frame_level_recurrent_objects(data, downsampling=downsampling)
        if model_name in {'GitHOI', '2G-GCN', 'TIGAhoi'}:
            if sigma:
                ys[2] = ignore_last_step_end_flag_general(ys[2])
            ys[2] = smooth_segmentation(ys[2], sigma)
            ys_budget = ys[2]
            xs_hh_dists = bimanual_interactions.assemble_human_human_distances(data, downsampling=downsampling)
            xs_ho_dists = bimanual_interactions.assemble_human_object_distances(data, downsampling=downsampling)
            xs_oo_dists = bimanual_interactions.assemble_object_object_distances(data, downsampling=downsampling)
            xs_steps = assemble_num_steps(data, downsampling=downsampling)
            action_text = bimanual_interactions.assemble_text_features(data)
            caption = bimanual_interactions.assemble_caption_features(data, downsampling=downsampling)
            xs = xs[:1] + xs_objects + xs[1:] + [xs_hh_dists, xs_ho_dists, xs_oo_dists, xs_steps] + [caption] + [action_text]
            ys = [ys_budget] + ys[2:] + ys[:2]
            ys += ys[-2:]
        elif model_name == 'bimanual_baseline':
            ...
            raise ValueError(f'BimanualInteractions code not implemented for {model_name} yet.')
        return xs, ys
    ...
    def assemble_frame_level_recurrent_human(data, downsampling: int = 1, test_data: bool = False):
        ...
        for left_hand, right_hand, _, _, _, _, objects_bounding_box, left_hand_pose, right_hand_pose, _, _ in data:
            ...
            xs_lh.append(left_hand)
            xs_rh.append(right_hand)
            left_hand_pose = left_hand_pose[downsampling - 1::downsampling] / 1000
            right_hand_pose = right_hand_pose[downsampling - 1::downsampling] / 1000
            xs_lhp.append(left_hand_pose)
            xs_rhp.append(right_hand_pose)
            objects_bounding_box = objects_bounding_box[downsampling - 1::downsampling] / 1000
            x_obb.append(objects_bounding_box)

        max_no_objects = max(5,max(video.shape[1] for video in x_obb)) if x_obb else 0
        # max_no_objects = min(7, max(video.shape[1] for video in x_obb)) if x_obb else 0
        bbb = []
        for video in x_obb:
            bb = []
            for frame in video:
                b = np.zeros((max_no_objects, 4), dtype=np.float32)
                n = frame.shape[0]
                if n < max_no_objects:
                    b[:n] = frame
                else:
                    b = frame[:max_no_objects]
        ...
        for i, video in enumerate(xs_lh):
            l, r = [], []
            for j, frame in enumerate(video):
                lhp = xs_lhp[i][j][keypoints]
                rhp = xs_rhp[i][j][keypoints]
                obb = xs_obb[i][j]
                if j + 1 < len(video):
                    next_lhp = xs_lhp[i][j + 1][keypoints]
                    lhp_velo = (next_lhp - lhp) * 100
                    ...
                    lhp_velo = np.zeros_like(lhp, dtype=np.float32)
                    rhp_velo = np.zeros_like(rhp, dtype=np.float32)
                    obb_velo = np.zeros_like(obb, dtype=np.float32)
                lhpvelo = np.hstack((lhp, lhp_velo))
                rhpvelo = np.hstack((rhp, rhp_velo))
                obbvelo = np.hstack((obb, obb_velo))
                lpv = lhpvelo.reshape(1, -1)
                rpv = rhpvelo.reshape(1, -1)
                obv = obbvelo.reshape(1, -1)
                context = np.concatenate((lpv[0], rpv[0], obv[0]))
                lh_con = np.concatenate((frame, context))
                rh_con = np.concatenate((xs_rh[i][j], context))
                l.append(lh_con)
                r.append(rh_con)
    ...
    def assemble_caption_features(data, downsampling: int = 1):
        ...
        max_len = 0
        for _, _, _, _, _, _, _, _, _, caption, _ in data:
            caption_downsampled = caption[downsampling - 1::downsampling]
            max_len = max(max_len, caption_downsampled.shape[0])
            caption_features.append(caption_downsampled)
        feature_size = caption_features[-1].shape[-1]
        x_caption = np.full([len(caption_features), max_len, feature_size],
                            fill_value=np.nan, dtype=np.float32)
        for m, x_c in enumerate(caption_features):
