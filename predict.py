...
from vhoi.my_data_loading import load_unseen_testing_data,load_testing_data, select_model_data_feeder, select_model_data_fetcher
...
class _FeedWrapper(torch.nn.Module):
    def __init__(self, model, feed_model_data, data):
        super().__init__()
        self.model = model
        self.feed_model_data = feed_model_data
        self.data = data

    def forward(self):
        return self.feed_model_data(self.model, self.data)


def benchmark_and_gflops(model, feed_model_data, sample_data, device: str, warmup_iters: int = 10, timed_iters: int = 30):
    ...
    if profile is not None:
        wrapper = _FeedWrapper(model, feed_model_data, sample_data)
        wrapper.eval()
        with torch.no_grad():
            macs, _ = profile(wrapper, inputs=(), verbose=False)
        batch_size = int(sample_data[0].shape[0]) if hasattr(sample_data[0], "shape") else 1
        gflops_total = float(macs) * 2.0 / 1e9
        gflops_per_sample = gflops_total / max(batch_size, 1)
    ...
    with torch.no_grad():
        for _ in range(max(warmup_iters, 0)):
            _ = feed_model_data(model, sample_data)
        _sync()
        t0 = time.perf_counter()
        for _ in range(max(timed_iters, 1)):
            _ = feed_model_data(model, sample_data)
        _sync()
        t1 = time.perf_counter()
    avg_ms_per_batch = (t1 - t0) * 1000.0 / max(timed_iters, 1)
    batch_size = int(sample_data[0].shape[0]) if hasattr(sample_data[0], "shape") else 1
    avg_ms_per_sample = avg_ms_per_batch / max(batch_size, 1)
    return {
        ...
        "avg_ms_per_sample": float(avg_ms_per_sample),
        "batch_size": int(batch_size),
    }

def predict_unseen(model_dir, frame_to_segment_level=False, inspect_model=False):
    ...
    test_loader, data_info, segmentations, test_ids = load_unseen_testing_data(cfg.data.data, model_name, model_input_type,
    ...
    dataset_name = cfg.data.data.get('name', default_value='cad120')
    ...
    downsampling = cfg.data.data.get('downsampling', default_value=1)
    try:
        first_batch = next(iter(test_loader))
    except StopIteration:
        first_batch = None
    if first_batch is not None:
        sample_data, _ = fetch_model_data(first_batch, device=device)
        stats = benchmark_and_gflops(model, feed_model_data, sample_data, device=device)
        print(f"Model Params (total/trainable): {stats['params_total']}/{stats['params_trainable']}")
        if stats["gflops_total"] is None:
            print("Model GFLOPs: unavailable (thop not installed)")
        else:
            print(f"Model GFLOPs (per-batch/per-sample): {stats['gflops_total']:.4f}/{stats['gflops_per_sample']:.4f}")
        print(f"Avg Inference Time ms (per-batch/per-sample, bs={stats['batch_size']}): {stats['avg_ms_per_batch']:.3f}/{stats['avg_ms_per_sample']:.3f}")
...
def predict(model_dir, frame_to_segment_level=False, inspect_model=False):
    ...
    checkpoint = torch.load(checkpoint_file, map_location=device)
    scalers = checkpoint.get('scalers', None)
    test_loader, data_info, segmentations, test_ids = load_testing_data(cfg.data.data, model_name, model_input_type,
                                                                        batch_size=128, scalers=scalers)
    # Load model
    Model = select_model(model_name)
    model_creation_args = cfg.models.parameters
    ...
    if dataset_name == 'bimanual':
        with open(cfg.data.data.video_id_to_video_fps, mode='r') as f:
...
def maybe_load_class_dictionaries(model_dir):
    ...
    dataset_name = cfg.data.data.get('name', default_value='cad120')
    if dataset_name == 'cad120':
        dictionaries_dir = os.path.dirname(cfg.data.data.video_id_to_subject_id)
        ...
        dictionaries_dir = os.path.dirname(cfg.data.data.video_id_to_video_fps)
    ...
    elif dataset_name == 'bimanual_interactions':
        dictionaries_dir = os.path.dirname(cfg.data.data.path)
        dictionary_path = os.path.join(dictionaries_dir, 'bimanual_action_id_to_action_name.json')
        with open(dictionary_path, mode='r') as f:
            subactivity_id_to_name = json.load(f)
        subactivity_id_to_name = {int(k): v for k, v in subactivity_id_to_name.items()}
    else:
...
def predict_all_unseen(args):
    pretrained_model_dir = args.pretrained_model_dir
    model_name = pretrained_model_dir.split('/')[3]
    ...
    if cross_validate:
        ...
        if model_name == '2G-GCN':
            model_id = '_'.join(model_id_parts[:5])
        elif model_name in {'GitHOI' 'TIGAhoi'}:
            model_id = '_'.join(model_id_parts[:8])
        dirname = os.path.dirname(pretrained_model_dir)
        outputs_per_subject = {}
        test_tasks = ['cheering', 'hair_cutting', 'co_working']
        for test_task in test_tasks:
            current_model_dir = os.path.join(dirname, model_id + '_' + test_task + '_' + str(radio))
            outputs, targets, test_ids = predict_unseen(current_model_dir, convert_frame_to_segment_level, inspect_model)
            try:
                outputs, targets, test_ids = predict_unseen(current_model_dir, convert_frame_to_segment_level, inspect_model)
            except FileNotFoundError:
                continue
            else:
                outputs_per_subject[test_task] = outputs, targets, test_ids
        results_per_subject = {}
...
class PREDICT():
    def __init__(self, pretrained_model_dir, cross_validate = True, convert_frame_to_segment_level = False, inspect_model = False):
        self.pretrained_model_dir = pretrained_model_dir
        self.cross_validate = cross_validate
        self.unseen_radio = 0.66
        self.convert_frame_to_segment_level = convert_frame_to_segment_level
        # self.save_visualisations_dir = './Predict/mphoi/ASSIGN/ASSIGN_hs512_e40_bs8_lr0.0001_0.1_Subject45'
        # self.save_visualisations_dir = './Output/Predict/mphoi/VISA-HOI/'
        self.save_visualisations_dir = './Output/Predict/cad120/M3-HOI/temp'
        # self.save_visualisations_dir = './Predict/cad120/MM-HOI/hs512_ems128_tk512_cnn[32, 256]_e40_bs16_lr0.0001_0.1_Subject1'
        # self.save_visualisations_dir = './Predict/bimanual/2G-GCN/hs64_e60_bs32_lr0.0001_0.1_1'
...
def main():
    ...
    # args.func(args)
    # args_predict = PREDICT( pretrained_model_dir = './Output/CheckSave/mphoi/VISA-HOI/Complete/cTrue_mtv3_nl1_hs512_e40_bs8_lr0.0001_0.1_Subject45')
    
    
    # args_predict = PREDICT( pretrained_model_dir = './outputs/bimanual/GitHOI/cTrue_mtv3_nl1_hs64_e40_bs32_lr0.0005_0.1_1')
    # args_predict = PREDICT( pretrained_model_dir = './outputs/cad120/2ST-Net/hs512_ems136_tk512_e40_bs16_lr5e-05_0.1_Subject1')
    
    # args_predict = PREDICT( pretrained_model_dir = './Output/CheckSave/mphoi/M3-HOI/Complete/FasterRCNN/TIGAhoi_False_cTrue_mtv3_nl1_cl2_hs512_e40_bs8_lr0.0001_0.5_Subject25')

    args_predict = PREDICT( pretrained_model_dir = './outputs/bimanual_interactions/TIGAhoi/TIGAhoi_True_cTrue_mtv3_nl1_cl2_hs64_e40_bs32_lr0.001_0.5_5')

    # predict_all_unseen(args_predict)
    predict_all(args_predict)
    # arg_parser = create_arg_parser()
    # args = arg_parser.parse_args()
    # args.func(args)
